...
! ###################################################################
      ...
      PROGRAM VLIDORT_LPS_SPHERCORR_TESTER_V2
...
!  11/28/22. Version 2.8.3 Upgrade.
!     Single call to sphericity correction master, instead of 3 separate calls for 2pt/3pt/Mpt
...
!    Applies equally to the Satellite view (upwelling) or the ground view (downwelling)
      ...
      REAL(FPK) :: FO_STOKES_BOAGEOM   ( MAXSTOKES, MAXGEO ) ! 1. FO (SS/DB) Stokes vector for BOA geo
      REAL(FPK) :: MS_STOKES_BOAGEOM   ( MAXSTOKES, MAXGEO ) ! 2. MS Stokes vector for BOA geo
      REAL(FPK) :: MS_STOKES_SPHERCORR ( MAXSTOKES, MAXGEO ) ! 3. MS Stokes vector with MS2pt corr
      REAL(FPK) :: STOKES_BOAGEOM      ( MAXSTOKES, MAXGEO ) ! 4. Full Stokes vector for BOA geo      4 = 1 + 2
      REAL(FPK) :: STOKES_SPHERCORR    ( MAXSTOKES, MAXGEO ) ! 5. Full Stokes vector with Spher corr  5 = 1 + 3

!  Profile Jacobian output with explanations

      REAL(fpk) :: FO_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 1. FO (SS/DB) LP Jac for BOA geo
      REAL(fpk) :: MS_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 2. MS LP Jac for BOA geo
      REAL(fpk) :: MS_PROFILEWF_SPHERCORR ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk) :: PROFILEWF_BOAGEOM      ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 4. Full LP Jac for BOA geo      4 = 1 + 2
      REAL(fpk) :: PROFILEWF_SPHERCORR    ( MAX_ATMOSWFS, MAXLAYERS, MAXSTOKES, MAXGEO ) ! 5. Full LP Jac with Spher corr  5 = 1 + 3

!  Surface Jacobian output with explanations

      REAL(fpk) :: FO_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 1. FO (SS/DB) LP Jac for BOA geo
      REAL(fpk) :: MS_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 2. MS LP Jac for BOA geo
      REAL(fpk) :: MS_SURFACEWF_SPHERCORR ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk) :: SURFACEWF_BOAGEOM      ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 4. Full LP Jac for BOA geo      4 = 1 + 2
      REAL(fpk) :: SURFACEWF_SPHERCORR    ( MAX_SURFACEWFS, MAXSTOKES, MAXGEO ) ! 5. Full LP Jac with Spher corr  5 = 1 + 3
...
!    ==> Twilight flag active when TOA-geometry SZA > 90 degrees (VLIDORT can't handle this)
      ...
      CHARACTER*256 :: Main_Message, Local_Message, Local_Action
...
!  Other help variables
      ...
      !                    ALBEDO
      REAL(FPK)        :: SZA, SZA_SET(3), VZA, VZA_SET(3), RAA, RAA_SET(3), &
                          ALBEDO
      CHARACTER*1      :: MSType
...
!      CMASK = (/  1, 5, 5, 2, 3, 6, 6, 4 /)
      ...
      SZA_SET = (/ 0.0_fpk, 40.0_fpk, 80.0_fpk  /)
      VZA_SET = (/ 0.0_fpk, 40.0_fpk, 80.0_fpk  /)
      RAA_SET = (/ 0.0_fpk, 90.0_fpk, 180.0_fpk /)
...
!  1/31/21. Version 2.8.3, New variables must be set by hand
      ...
      VLIDORT_FixIn%Cont%TS_ASYMTX_TOLERANCE = 1.0e-12_fpk
...
!  Identify Albedo file extension
      ...
      if ( Do_Albedo ) then
        write(CALB,'(I2.2)') INT(100.0_fpk*Albedo)
      else
        write(CALB,'(I2.2)') INT(100.0_fpk*VLIDORT_FixIn%Optical%TS_LAMBERTIAN_ALBEDO)
      endif
...
!  -------------------------------
      ...
      do n = 1, nlayers
        ...
        if(molomg(n).gt.0.99999999_fpk) molomg(n)=0.99999999_fpk
      enddo
      height_grid(0) = 60.0_fpk
      close(45)
...
!  Rayleigh scattering law
      ...
      depol = ( 1.0_fpk - 2.0_fpk*raymoms(2,NLAYERS) ) / &
              ( 1.0_fpk + raymoms(2,NLAYERS) )
      beta2 = raymoms(2,NLAYERS)
      PROBLEM_RAY = 0.0_fpk
      PROBLEM_RAY(1,0) =  1.0_fpk
      PROBLEM_RAY(4,1) =  3.0_fpk * ( 1.0_fpk - 2.0_fpk*DEPOL ) / (2.0_fpk + DEPOL )
      PROBLEM_RAY(1,2) =  beta2
      PROBLEM_RAY(5,2) =  - SQRT(6.0_fpk) * beta2
      PROBLEM_RAY(2,2) =  6.0_fpk * beta2
...
!  Wavelength. Override the value in the configuration file (dummy, not needed)
      ...
      VLIDORT_FixIn%Optical%TS_ATMOS_WAVELENGTH = 0.333_fpk
...
!  Fix optical properties for the non-aerosol layers
      ...
      do n = 1, n6
         ...
         DO K = 1, ngreekmat_entries
           ...
           ENDDO
           GREEKMAT_TOTAL_INPUT(0,n,1) = 1.0_fpk
         ENDDO
         ratio1 = 1.0_fpk - molomg(n)
...
!  Fix layers with aerosol
      ...
      if ( do_aerosols ) then
        ...
        do n = n6 + 1, nlayers
          ...
          L_omega_total_input(2,n) = ratio2 * ((waer/omega) - 1.0_fpk)
          DO K = 1, ngreekmat_entries
          ...
          DO K = 1, ngreekmat_entries
            ...
            do L = 1, N_AERMOMS
              ...
              IF ( MOM .ne.zero ) L_greekmat_total_input(2,L,n,GK) = ratio2 * ( (SK * aermoms(CK,L)/mom) - 1.0_fpk )
            enddo
          enddo
        enddo
      Endif
...
!  Set RAA according to loop value
      ...
      raa = raa_set(ia)
      if( ia.eq.1 ) raa = 0.001_fpk 
      if( ia.eq.NRAA ) raa = 179.999_fpk
...
!   sza = 90.0_fpk - 85.0_fpk
!   write(*,*)'Critang = ',atan(sin(sza*deg_to_rad)/(cos(sza*deg_to_rad)-(6371.0_fpk/6471.0_fpk)))/deg_to_rad
!   stop

!  11/28/22. Version 2.8.3 Upgrade. Single call, instead of 3 separate calls.

      CALL SPHERICITY_CORRECTION_LPS_MASTER ( do_debug_input, &
            VLIDORT_FixIn, VLIDORT_ModIn, VLIDORT_Sup, MSType,                   & ! VLIDORT Standard   Inputs
...
!  11/28/22. Version 2.8.3 Upgrade. Also output Main_message.

      IF ( Fail ) THEN
        if ( Twilight_flag ) then
          write(*,'(A)')'Sphericity correction failed as the geometry is invalid (twilight flag)'
        else
          write(*,'(A)')'Sphericity correction failed due to VLIDORT error - here are the messages'
          write(*,'(A)')'  --'//Trim(Main_Message)
          write(*,'(A)')'  --'//Trim(Local_Message)
          write(*,'(A)')'  --'//Trim(Local_Action)
          write(*,'(A)')'Now look at "VLIDORT_LPS_SpherCorr_tester_V2.log" file for error details'
          CALL VLIDORT_WRITE_STATUS ('VLIDORT_LPS_SpherCorr_tester_V2.log', VLIDORT_ERRUNIT, OPENFILEFLAG, VLIDORT_Out%Status )
        endif
      ...
      if ( .not. Twilight_Flag ) then
        ...
        DO iv = 1,NVZA
          ...
          write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  I  ',&
              ...
              100.0_fpk*(1.0_fpk-(MS_PROFILEWF_SPHERCORR(1,lay,1,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,1,igeo))),&
              100.0_fpk*(1.0_fpk-(PROFILEWF_SPHERCORR(1,lay,1,igeo)/PROFILEWF_BOAGEOM(1,lay,1,igeo)))
        ENDDO ; ENDDO ; ENDDO
        ...
        if (nstokes.gt.1) then
          ...
          DO iv = 1,NVZA
            ...
            write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  Q  ',&
                ...
                100.0_fpk*(1.0_fpk-(MS_PROFILEWF_SPHERCORR(1,lay,2,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,2,igeo))),&
                100.0_fpk*(1.0_fpk-(PROFILEWF_SPHERCORR(1,lay,2,igeo)/PROFILEWF_BOAGEOM(1,lay,2,igeo)))
          ENDDO ; ENDDO ; ENDDO
        endif
          ...
          DO iv = 1,NVZA
            ...
            if (abs(PROFILEWF_BOAGEOM(1,lay,3,igeo)) .gt. ZERO) then
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ',&
                  ...
                  100.0_fpk*(1.0_fpk-(MS_PROFILEWF_SPHERCORR(1,lay,3,igeo)/MS_PROFILEWF_BOAGEOM(1,lay,3,igeo))),&
                  100.0_fpk*(1.0_fpk-(PROFILEWF_SPHERCORR(1,lay,3,igeo)/PROFILEWF_BOAGEOM(1,lay,3,igeo)))
            else
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ', &
                  FO_PROFILEWF_BOAGEOM(1,lay,3,igeo), MS_PROFILEWF_BOAGEOM(1,lay,3,igeo), PROFILEWF_BOAGEOM(1,lay,3,igeo), &
                  MS_PROFILEWF_SPHERCORR(1,lay,3,igeo), PROFILEWF_SPHERCORR(1,lay,3,igeo),&
...
155   format(1x,i3,3f8.3,a5,7(1x,es15.7))
...
!  Finish
      ...
      write(*,'(/a/)')'Main program VLIDORT_LPS_SPHERCORR_TESTER_V2 finished successfully'

      END PROGRAM VLIDORT_LPS_SPHERCORR_TESTER_V2
