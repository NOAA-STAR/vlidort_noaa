...
! ###################################################################
      ...
      PROGRAM VLIDORT_LCS_SPHERCORR_TESTER_V2
...
!  1/31/21. Version 2.8.3, New variables must be set by hand
      ...
      VLIDORT_FixIn%Cont%TS_ASYMTX_TOLERANCE = 1.0e-20_fpk
...
!     Gamma-distributed, 0.782 Microns, n = 1.43, reff = 1.05, veff = 0.07
      ...
      if ( do_aerosols ) then
        ...
        do L = 1, 5
          read(1,*)
        enddo
        LCHECK = 0 ; L = -1 ; cutoff = 1.0e-06_fpk
...
!  11/28/22. Version 2.8.3 Upgrade. Single call, instead of 3 separate calls.
      ...
      CALL SPHERICITY_CORRECTION_LCS_MASTER ( do_debug_input, &
...
!  11/28/22. Version 2.8.3 Upgrade. Also output Main_message.
      ...
      IF ( Fail ) THEN
        if ( Twilight_flag ) then
          ...
          write(*,'(A)')'Now look at "VLIDORT_LCS_SpherCorr_tester_V2.log" file for error details'
          CALL VLIDORT_WRITE_STATUS ('VLIDORT_LCS_SpherCorr_tester_V2.log', VLIDORT_ERRUNIT, OPENFILEFLAG, VLIDORT_Out%Status )
        endif
      ...
      if ( .not. Twilight_Flag ) then
        ...
        DO iv = 1,NVZA
          ...
          write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  I  ',&
              ...
              100.0_fpk*(1.0_fpk-(MS_COLUMNWF_SPHERCORR(1,1,igeo)/MS_COLUMNWF_BOAGEOM(1,1,igeo))),&
              100.0_fpk*(1.0_fpk-(COLUMNWF_SPHERCORR(1,1,igeo)/COLUMNWF_BOAGEOM(1,1,igeo)))
        ENDDO ; ENDDO ; ENDDO
        ...
        if (nstokes.gt.1) then
          ...
          DO iv = 1,NVZA
            ...
            write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  Q  ',&
                ...
                100.0_fpk*(1.0_fpk-(MS_COLUMNWF_SPHERCORR(1,2,igeo)/MS_COLUMNWF_BOAGEOM(1,2,igeo))),&
                100.0_fpk*(1.0_fpk-(COLUMNWF_SPHERCORR(1,2,igeo)/COLUMNWF_BOAGEOM(1,2,igeo)))
          ENDDO ; ENDDO ; ENDDO
        endif
          ...
          DO iv = 1,NVZA
            ...
            if (abs(COLUMNWF_BOAGEOM(1,3,igeo)) .gt. ZERO) then
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ',&
                  ...
                  100.0_fpk*(1.0_fpk-(MS_COLUMNWF_SPHERCORR(1,3,igeo)/MS_COLUMNWF_BOAGEOM(1,3,igeo))),&
                  100.0_fpk*(1.0_fpk-(COLUMNWF_SPHERCORR(1,3,igeo)/COLUMNWF_BOAGEOM(1,3,igeo)))
            else
              write(701,155) igeo,sza_set(is),vza_set(iv),raa_set(ia),'  U  ', &
                  FO_COLUMNWF_BOAGEOM(1,3,igeo), MS_COLUMNWF_BOAGEOM(1,3,igeo), COLUMNWF_BOAGEOM(1,3,igeo), &
                  MS_COLUMNWF_SPHERCORR(1,3,igeo), COLUMNWF_SPHERCORR(1,3,igeo),&
...
!  Finish
      ...
      write(*,'(/a/)')'Main program VLIDORT_LCS_SPHERCORR_TESTER_V2 finished successfully'

      END PROGRAM VLIDORT_LCS_SPHERCORR_TESTER_V2
