...
! #                                                             #
! #          SPHERICITY_CORRECTION_LPS_MASTER (master, public)  #
! #          MS2pt_LPS_SPHERCORR_V1                             #
...
!  This is the LPS linearization of the sphericity correction
...
!  11/28/22. Upgrade to Version 2.8.3.
!    -- NEW LPS_master routine is public

public  ::  SPHERICITY_CORRECTION_LPS_MASTER

!  All three MSType routines are private

private :: MS2pt_LPS_SPHERCORR_V1, &
           MS3pt_LPS_SPHERCORR_V1, &
           MSMpt_LPS_SPHERCORR_V1

contains

SUBROUTINE SPHERICITY_CORRECTION_LPS_MASTER  ( do_debug_input, & 
...
!  Call to dedicated LCS_sphericity correction routine (either 2-point, 3-point or multi-point)
      ...
      if ( MSType .eq. '2' ) then
        ...
        Call MS3pt_LPS_SPHERCORR_V1 ( do_debug_input, & 
            ...
            VLIDORT_Out, VLIDORT_LinOut, STOKES_BOAGEOM, STOKES_SPHERCORR,            & ! VLIDORT and Standard SpherCorr output
            PROFILEWF_BOAGEOM, PROFILEWF_SPHERCORR, SURFACEWF_BOAGEOM, SURFACEWF_SPHERCORR, & ! Linearized SpherCorr output
            Twilight_Flag, Fail, Local_Message, Local_Action )                                ! Exceptions and error
...
!  Exception handling
      ...
      IF ( Fail ) THEN
        Main_message = 'Failure from Call to MS'//MStype//'pt_LPS_SPHERICITY_CORRECTION'
      endif

!  return

      return
END SUBROUTINE SPHERICITY_CORRECTION_LPS_MASTER
...
!  Standard output with explanations
      ...
      REAL(fpk), INTENT(INOUT) :: FO_STOKES_BOAGEOM(4)   ! 1. First-order (SS/DB) STOKES Vector for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_STOKES_BOAGEOM(4)   ! 2. MS STOKES Vector for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_STOKES_SPHERCORR(4) ! 3. MS STOKES Vector with 2-point corr
      REAL(fpk), INTENT(INOUT) :: STOKES_BOAGEOM(4)      ! 4. Full STOKES for BOA geo.          4 = 1 + 2.
      REAL(fpk), INTENT(INOUT) :: STOKES_SPHERCORR(4)    ! 5. Full STOKES with Sphericity corr. 5 = 1 + 3.

!  Profile Jacobian output with explanations

      REAL(fpk), INTENT(INOUT) :: FO_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 1. First-order (SS/DB) LP Jac for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_PROFILEWF_BOAGEOM   ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 2. MS LP Jac for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_PROFILEWF_SPHERCORR ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 3. MS LP Jac with 2-point corr
      REAL(fpk), INTENT(INOUT) :: PROFILEWF_BOAGEOM      ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 4. Full LP Jac for BOA geo.     4 = 1 + 2.
      REAL(fpk), INTENT(INOUT) :: PROFILEWF_SPHERCORR    ( MAX_ATMOSWFS, MAXLAYERS, 4 ) ! 5. Full LP Jac with MS2pt corr. 5 = 1 + 3.

!  Surface Jacobian output with explanations

      REAL(fpk), INTENT(INOUT) :: FO_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 1. First-order (SS/DB) LC Jac for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 2. MS LS Jac for BOA geo
      REAL(fpk), INTENT(INOUT) :: MS_SURFACEWF_SPHERCORR ( MAX_SURFACEWFS, 4 ) ! 3. MS LS Jac with 2-point corr
      REAL(fpk), INTENT(INOUT) :: SURFACEWF_BOAGEOM      ( MAX_SURFACEWFS, 4 ) ! 4. Full LS Jac for BOA geo.     4 = 1 + 2.
      REAL(fpk), INTENT(INOUT) :: SURFACEWF_SPHERCORR    ( MAX_SURFACEWFS, 4 ) ! 5. Full LS Jac with MS2pt corr. 5 = 1 + 3.
