...
!    -- (2/16/21). Use of the FOURIER0_NSTOKES2 option 
      ...
      SUBROUTINE VLIDORT_LPS_MASTER ( do_debug_input, &
        VLIDORT_FixIn, &
        VLIDORT_ModIn, &
        VLIDORT_Sup, &
        VLIDORT_Out, &
...
!  1/31/21. Version 2.8.3. Use VLIDORT_LPS_CONVERGE_m for the three convergence routines
...
!  1/31/21. Version 2.8.3. Installed MSST LP and LS linearizations
!    ==> Additional layer_mssts and surf_mssts, Fourier component input (upwelling case)
!    ==> MSST situation expanded to include Downwelling case (as an alternative)

      DOUBLE PRECISION :: LP_SURF_MSSTS_F  ( MAXBEAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: LP_LAYER_MSSTS_F ( MAXBEAMS, MAXSTOKES, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
...
!      DOUBLE PRECISION :: PROFILEWF ( MAX_ATMOSWFS, MAXLAYERS, MAX_USER_LEVELS, MAX_GEOMETRIES, MAXSTOKES, MAX_DIRECTIONS )
...
!    ==> Added LOSTRANS_DN for the Downwelling Alternative.
      ...
      DOUBLE PRECISION :: FO_LP_LOSTRANS_UP ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION :: FO_LP_LOSTRANS_DN ( MAX_GEOMETRIES, MAXLAYERS, MAX_ATMOSWFS )
...
!  Misc. help
      ...
      INTEGER ::          OFF, O1, UA, UM, IB, G, G1, G2, N, K, L, Q, Q1, UTA, LUM, LUA, NM, NMOMS, NWFS
...
!mick fix 2/7/2022 - TS version of status vars now set here

!  Input checks

      !STATUS_INPUTCHECK = VLIDORT_SUCCESS
      VLIDORT_Out%Status%TS_STATUS_INPUTCHECK = VLIDORT_SUCCESS
...
!  Main calculation
      ...
      VLIDORT_Out%Status%TS_STATUS_CALCULATION = VLIDORT_SUCCESS
...
!  Regular dimensions
      ...
      ( VLIDORT_FixIn, VLIDORT_ModIn, &
        STATUS_INPUTCHECK, NCHECKMESSAGES, CHECKMESSAGES, ACTIONS )
...
!mick mod 9/22/2022 - added input variable "ATMOS_INDEX"
      ...
      ATMOS_INDEX             = VLIDORT_FixIn%Optical%TS_ATMOS_INDEX
...
!mick fix 9/19/2017 - swapped layer & geo indices
!mick fix 1/5/2021  - replaced N_TOTALCOLUMN_WFS with N_TOTALPROFILE_WFS in WF dimension

      !L_FMATRIX_UP ( 1:N_TOTALPROFILE_WFS,:,1:NLAYERS,:) = &
      !  VLIDORT_LinFixIn%Optical%L_FMATRIX_UP(1:N_TOTALPROFILE_WFS,:,1:NLAYERS,:)
      !L_FMATRIX_DN ( 1:N_TOTALPROFILE_WFS,:,1:NLAYERS,:) = &
      !  VLIDORT_LinFixIn%Optical%L_FMATRIX_DN(1:N_TOTALPROFILE_WFS,:,1:NLAYERS,:)
      L_FMATRIX_UP (1:N_TOTALPROFILE_WFS,1:NLAYERS,:,:) = &
        VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_UP(1:N_TOTALPROFILE_WFS,1:NLAYERS,:,:)
      L_FMATRIX_DN (1:N_TOTALPROFILE_WFS,1:NLAYERS,:,:) = &
        VLIDORT_LinFixIn%Optical%TS_L_FMATRIX_DN(1:N_TOTALPROFILE_WFS,1:NLAYERS,:,:)
...
!  --------------------
      ...
      IF ( .not.DO_FOCORR_EXTERNAL ) THEN
         VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES,:) = zero
         VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,:,1:NSTOKES)   = zero
...
!   -- Several restrictions on use of MSSTS option, need to be checked
      ...
      CALL VLIDORT_CHECK_INPUT ( &
           ...
           STATUS_INPUTCHECK, NCHECKMESSAGES, CHECKMESSAGES, ACTIONS )                 ! Exception handling
...
!  mick mod 2/7/2022 - these outputs now set at the end of input checking
!          VLIDORT_Out%Status%TS_NCHECKMESSAGES = NCHECKMESSAGES
!          VLIDORT_Out%Status%TS_CHECKMESSAGES  = CHECKMESSAGES
!          VLIDORT_Out%Status%TS_ACTIONS        = ACTIONS
!  ########## Rob Change, 3/28/2011 ################
...
!mick fix 2/7/2022 - define current errors using the local versions of the check vars and then
!                    pass all messages (including possible warnings) to the TS error handling outputs
      ...
      IF ( DO_PLANETARY_PROBLEM .OR. DO_ALBTRN_MEDIA(1) .OR. DO_ALBTRN_MEDIA(2) ) then
         IF ( (.NOT. DO_LAMBERTIAN_SURFACE) .OR. DO_SURFACE_LEAVING .OR. DO_THERMAL_EMISSION &
            ...
            NM = NCHECKMESSAGES
            NCHECKMESSAGES      = NM+2
            CHECKMESSAGES(NM+1) = 'Media/Planetary problem: input check not valid'
            ACTIONS(NM+1)       = 'Either: Turn off Thermal_Emission/FOCORR_OUTGOING/Surface_Leaving flags'
            ACTIONS(NM+2)       = 'Or    : Make sure Lambertian surface flag and set albedo to zero'
...
!  New code-----------
      IF ( STATUS_INPUTCHECK .EQ. VLIDORT_WARNING ) THEN
        VLIDORT_Out%Status%TS_STATUS_INPUTCHECK = VLIDORT_WARNING
...
!mick mod 2/7/2022 - since this is the last check sub, modified exception handling
!                    to define TS versions of the input check vars here and not at
!                    end of master in case of a calc failure later (resulting in
!                    an undefined condition for the TS input check vars)
...
!    ==> MSST situations: Add LOSTRANS_UP/DN, LP_LOSTRANS_UP/DN, THETA_ALL, ALPHA to output list
...
!  1/31/21. Version 2.8.3. Add Doublet geometry flag and include offsets
            ...
            CALL VFO_LPS_MASTER_INTERFACE ( &
                ...
                FO_PROFILEWF_SS,  FO_PROFILEWF_DB, FO_PROFILEWF_DTA, FO_PROFILEWF_DTS,              & ! Output - Column Jacobians
                ...
                FO_LP_CUMTRANS, FO_LSSL_SLTERM, FO_LP_LOSTRANS_UP, FO_LP_LOSTRANS_DN,               & ! Output - Auxiliary
...
!mick fix 1/5/2021   - added defining of VLIDORT_Sup STOKES_SS & STOKES_DB and VLIDORT_LinSup PROFILEWF_SS, PROFILEWF_DB,
!                      & SURFACEWF_DB
            ...
            IF ( DO_UPWELLING ) THEN
               ...
               IF ( DO_PROFILE_LINEARIZATION ) THEN
                  VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS&
                                           (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,UPIDX) = &
                         FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,UPIDX)
                  VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB&
                                           (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES) = &
            ...
            IF ( DO_DNWELLING ) THEN
               ...
               IF ( DO_PROFILE_LINEARIZATION ) THEN
                 VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS&
                                          (1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,DNIDX) = &
                        FO_PROFILEWF_ATMOS(1:N_TOTALPROFILE_WFS,1:NLAYERS,1:N_USER_LEVELS,1:N_GEOMETRIES,1:NSTOKES,DNIDX)
               ENDIF
            ENDIF
      ...
      IF ( DO_MSSTS ) THEN
            ...
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              DO N = 1, NLAYERS
                DO Q = 1, LAYER_VARY_NUMBER(N)
                  VLIDORT_LinOut%Prof%TS_LP_LOSTRANS(Q,1:N_SZANGLES,N) = FO_LP_LOSTRANS_UP(1:N_SZANGLES,N,Q)
                ENDDO
              ENDDO
            ENDIF
         ELSE IF ( DO_DNWELLING ) THEN
            ...
            IF ( DO_PROFILE_LINEARIZATION ) THEN
              DO N = 1, NLAYERS
                DO Q = 1, LAYER_VARY_NUMBER(N)
                  VLIDORT_LinOut%Prof%TS_LP_LOSTRANS(Q,1:N_SZANGLES,N) = FO_LP_LOSTRANS_DN(1:N_SZANGLES,N,Q)
                ENDDO
              ENDDO
            ENDIF
         ENDIF
...
!  Skip Fourier loop if only the FO is required
!mick fix 2/7/2022 - added this IF block.
...
!mick mod 9/22/2022 - added input variable "ATMOS_INDEX" to argument list

        CALL VLIDORT_LPS_FOURIER ( ATMOS_INDEX, FOURIER, DO_FOURIER0_NSTOKES2, &
            ...
            PROFILEWF_F, MEANST_DIFFUSE_PROFWF, FLUX_DIFFUSE_PROFWF, DNMEANST_DIRECT_PROFWF, DNFLUX_DIRECT_PROFWF,  & !Out PROF JACS
            ...
            LAYER_MSSTS_F, SURF_MSSTS_F, LP_LAYER_MSSTS_F, LP_SURF_MSSTS_F, LS_LAYER_MSSTS_F, LS_SURF_MSSTS_F,      & !Output SPECIAL
...
!  error handling
        ...
        IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
          ...
          VLIDORT_Out%Status%TS_STATUS_CALCULATION = VLIDORT_SERIOUS
          VLIDORT_Out%Status%TS_MESSAGE = MESSAGE
          VLIDORT_Out%Status%TS_TRACE_1 = TRACE_1
          VLIDORT_Out%Status%TS_TRACE_2 = TRACE_2
          VLIDORT_Out%Status%TS_TRACE_3 = TRACE_3
...
!       -- LP_SLTERM_LOCAL must go inside the geometry loop, and FO_SLTERM must use index g (not g1)
...
!                       LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(O1,G1) * LP_TRANS_ATMOS_FINAL(IB,K,1:NWFS)
                       DO UTA = 1, N_USER_LEVELS ;  DO G = G1, G2
                          LP_SLTERM_LOCAL(1:NWFS) = FO_SLTERM(O1,G) * LP_TRANS_ATMOS_FINAL(IB,K,1:NWFS)
                          DO Q = 1, NWFS
...
!    -- Use/Fill linearized type structure variables directly (replaces PROFILENWF, SURFACEWF, associated FO inputs)
!    -- Drop LOCAL_UM_START from the Lattice converge routines.
!mick mod 9/22/2022 - added input variable "ATMOS_INDEX" to convergence argument lists

               IF ( DO_OBSERVATION_GEOMETRY ) THEN

!  Fourier summation of regular quantities

                 CALL VLIDORT_CONVERGE_OBSGEO ( ATMOS_INDEX, &
...
!   -- 5/24/21. Version 2.8.3. Add DO_INCLUDE_SLEAVEWFS (for surface-leaving weighting functions)
                 ...
                 IF ( DO_LINEARIZATION ) THEN
                   CALL VLIDORT_LPS_CONVERGE_OBSGEO ( ATMOS_INDEX, &
                     ...
                     DO_INCLUDE_SLEAVEWFS, AZMFAC, WHICH_DIRECTIONS, PROFILEWF_F, SURFACEWF_F,  & ! Input Bookkeeping/Fourier Jacs
...
!  Fourier summation of regular quantities
                 ...
                 CALL VLIDORT_CONVERGE_DOUBLET ( ATMOS_INDEX, &
...
!   -- 5/24/21. Version 2.8.3. Add DO_INCLUDE_SLEAVEWFS (for surface-leaving weighting functions)
                 ...
                 IF ( DO_LINEARIZATION ) THEN
                   CALL VLIDORT_LPS_CONVERGE_DOUBLET ( ATMOS_INDEX, &
                      DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,     & ! Input flags
                      DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,    & ! Input flags
                      ...
                      PROFILEWF_F, SURFACEWF_F, VLIDORT_LinSup%SS, VLIDORT_LinOut )                   ! Input/Output fields
                 ENDIF
...
!  Fourier summation of regular quantities
                 ...
                 CALL VLIDORT_CONVERGE ( ATMOS_INDEX, &
                   DO_FOCORR, DO_FOCORR_ALONE, DO_DBCORRECTION, DO_UPWELLING, DO_NO_AZIMUTH, & ! Input flags
                   DO_RAYLEIGH_ONLY, DO_ALL_FOURIER, DO_DOUBLE_CONVTEST, DO_TOA_CONTRIBS,    & ! Input flags
...
!   -- 5/24/21. Version 2.8.3. Add DO_INCLUDE_SLEAVEWFS (for surface-leaving weighting functions)
                 ...
                 IF ( DO_LINEARIZATION ) THEN
                   CALL VLIDORT_LPS_CONVERGE ( ATMOS_INDEX, &
                     DO_FOCORR, DO_FOCORR_ALONE, DO_UPWELLING, DO_NO_AZIMUTH, DO_DBCORRECTION,     & ! Input flags
                     DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, DO_LAMBERTIAN_SURFACE,    & ! Input flags
                     IBEAM, FOURIER, NSTOKES, NLAYERS, N_OUT_STREAMS, N_USER_LEVELS, N_DIRECTIONS, & ! Input control numbers
                     LAYER_VARY_FLAG, LAYER_VARY_NUMBER, N_SURFACE_WFS, N_SLEAVE_WFS,              & ! Input linearization control
...
!  End of ".NOT.DO_FOCORR_ALONE" IF block (includes Fourier loop)
!mick fix 2/7/2022 - added this line
...
!  Profile weighting functions
!    Output direct Mean/Flux added 17 May 2012
!mick fix 1/5/2021 - shut off passing of PROFILEWF - output type structure now filled directly
...
!mick mod 1/5/2021 - added IF conditions to only fill what you need (as in 2.8.3 "vlidort_master")
      ...
      IF ( DO_ALBTRN_MEDIA(1) ) THEN
         ...
         VLIDORT_LinOut%Prof%TS_ALBMED_FLUXES_PROFWF(1:NSTOKES,:,1:NLAYERS,:)               = &
                                    LP_ALBMED_FLUXES(1:NSTOKES,:,1:NLAYERS,:)
      ENDIF
      IF ( DO_ALBTRN_MEDIA(2) ) THEN
         ...
         VLIDORT_LinOut%Prof%TS_TRNMED_FLUXES_PROFWF(1:NSTOKES,:,1:NLAYERS,:)               = &
                                    LP_TRNMED_FLUXES(1:NSTOKES,:,1:NLAYERS,:)
      ENDIF
         ...
         ELSE IF ( DO_DOUBLET_GEOMETRY ) THEN
            DO IB =1, NBEAMS ; DO UM = 1, N_USER_VZANGLES ; DO O1 = 1, NSTOKES
               ...
               DO K = 1, nlayers ; IF ( layer_Vary_Flag(k) ) then ; DO Q = 1, layer_Vary_Number(k)
                  ...
                  VLIDORT_LinOut%Prof%TS_PLANETARY_TRANSTERM_PROFWF(O1,OFF+UM,K,Q) = L_TRANS(Q)
               ENDDO ; ENDIF ; ENDDO
            ENDDO ; ENDDO ; ENDDO
         ELSE
            DO IB =1, NBEAMS ; DO UM = 1, N_USER_VZANGLES ; DO O1 = 1, NSTOKES
...
!mick fix 1/5/2021 - shut off passing of PROFILEWF_SS, PROFILEWF_DB, & SURFACEWF_DB - output type structures now filled directly
...
!  Exception handling
!mick fix 2/7/2022 - input check and output calc TS status vars STATUS_INPUTCHECK and
!                    STATUS_CALCULATION now defined at top of master
!                  - input check TS message vars now defined after the last check
!                    subroutine in this master
...
!mick mod 9/22/2022 - added input variable "ATMOS_INDEX" to all input debug subs
      ...
      CALL VLIDORT_WRITE_STD_INPUT ( DO_DEBUG_WRITE, &
        ...
        ASYMTX_TOLERANCE, N_SZANGLES, N_USER_RELAZMS, N_USER_VZANGLES, N_USER_OBSGEOMS, N_USER_LEVELS,      & ! Geo/Output control
        SZANGLES,   USER_RELAZMS,   USER_VZANGLES,   USER_OBSGEOMS,   USER_LEVELS,                          & ! Geo/Output control
        HEIGHT_GRID, PRESSURE_GRID, TEMPERATURE_GRID, FINEGRID, GEOMETRY_SPECHEIGHT, ATMOS_WAVELENGTH,      & ! Atmos Inputs
        ATMOS_INDEX, DELTAU_VERT_INPUT, OMEGA_TOTAL_INPUT, GREEKMAT_TOTAL_INPUT, FMATRIX_UP, FMATRIX_DN,    & ! Atmos/Optical Inputs
...
!mick mod 9/19/2017 - added NMOMENTS to input. No longer required
      ...
      IF (.NOT. DO_LAMBERTIAN_SURFACE) THEN
        CALL VLIDORT_WRITE_SUP_BRDF_INPUT ( ATMOS_INDEX, &
...
!  1/31/21. Version 2.8.3. (2/16/21). SS arrays input directly from type structure. 
      ...
      IF (DO_FOCORR_EXTERNAL) THEN
        CALL VLIDORT_WRITE_SUP_SS_INPUT ( ATMOS_INDEX, &
          NSTOKES, N_USER_LEVELS, &
          VLIDORT_Sup%SS%TS_STOKES_SS, VLIDORT_Sup%SS%TS_STOKES_DB )
      END IF

!  1/31/21. Version 2.8.3. (2/16/21). SLEAVE arrays input directly from type structure. Restore Full Fourier loops

      IF (DO_SURFACE_LEAVING) THEN
        CALL VLIDORT_WRITE_SUP_SLEAVE_INPUT ( ATMOS_INDEX, &
...
!  3/28/14. Changes for Version 2.7. remove LTE linearization references. Add LBBF
      ...
      CALL VLIDORT_WRITE_LIN_INPUT ( ATMOS_INDEX, &
...
!mick mod 9/19/2017 - added NMOMENTS to input. No longer required
      ...
      IF (.NOT. DO_LAMBERTIAN_SURFACE .AND. DO_SURFACE_LINEARIZATION) THEN
        CALL VLIDORT_WRITE_LIN_SUP_BRDF_INPUT ( ATMOS_INDEX, &
...
!    -- Follow the LIDORT pattern, dedicated subroutine only for profile output.
      ...
      IF (DO_FOCORR_EXTERNAL) THEN
        CALL VLIDORT_WRITE_LPS_SUP_SS_INPUT ( ATMOS_INDEX, &
          DO_PROFILE_LINEARIZATION, DO_SURFACE_LINEARIZATION, NSTOKES,    &
          NLAYERS, N_USER_LEVELS, N_TOTALPROFILE_WFS, N_TOTALSURFACE_WFS, &
          VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_SS, &
          VLIDORT_LinSup%SS%Prof%TS_PROFILEWF_DB, &
...
!  1/31/21. Version 2.8.3. (2/16/21). SLEAVE arrays input directly from type structure. Restore Full Fourier loops
      ...
      IF (DO_SURFACE_LEAVING .AND. DO_SURFACE_LINEARIZATION .AND. DO_SLEAVE_WFS) THEN
        CALL VLIDORT_WRITE_LIN_SUP_SLEAVE_INPUT ( ATMOS_INDEX, &
...
!    - (5/24/21). Add DO_INCLUDE_SLEAVEWFS
      ...
      SUBROUTINE VLIDORT_LPS_FOURIER ( AWI, FOURIER, DO_FOURIER0_NSTOKES2,  &
...
!  Index of current atmospheric wvl or wvn

      INTEGER, INTENT (IN) ::          AWI
...
!    ==> MSST situation expanded to include Downwelling case (as an alternative)
      ...
      DOUBLE PRECISION, INTENT (OUT) :: LP_SURF_MSSTS_F  ( MAXBEAMS, MAXSTOKES, MAXLAYERS, MAX_ATMOSWFS )
      DOUBLE PRECISION, INTENT (OUT) :: LP_LAYER_MSSTS_F ( MAXBEAMS, MAXSTOKES, MAXLAYERS, MAXLAYERS, MAX_ATMOSWFS )
...
!  Indices and help for the Planetary problem
      ...
      INTEGER ::          I, IBEAM, IPARTIC, LAYER, N, NV, O1, K, K0, K1, K2, KO1, LUM, UM, Q
        ...
        IF ( DO_ALBTRN_MEDIA(1) .OR. DO_ALBTRN_MEDIA(2) .OR. DO_PLANETARY_PROBLEM ) THEN
           IF ( DO_PROFILE_LINEARIZATION ) THEN
              CALL VLIDORT_LP_MediaProps &
               ( DO_USER_STREAMS, DO_ALBTRN_MEDIA, LC_NSTK, NLAYERS, NSTREAMS,       & ! Input
                 N_USER_STREAMS, NSTREAMS_2, LC_NSTKNSTRM, LC_NSTKNSTRM_2,           & ! Input
...
!    -- (RTS 2/16/21). Use local bookkeeping variable LC_NSTK, as well as NSTOKES
        ...
        IF ( DO_UPWELLING ) THEN
          CALL VLIDORT_UPUSER_INTENSITY ( AWI, &
...
!    -- (RTS 2/16/21). Use local bookkeeping variable LC_NSTK, as well as NSTOKES
        ...
        IF ( DO_DNWELLING ) THEN
          CALL VLIDORT_DNUSER_INTENSITY ( AWI, &
...
!   -- (RTS 2/16/21). Use local bookkeeping variables LC_NSTK, and others
                   ...
                   CALL LP_BVP_SOLUTION_MASTER ( &
                      ...
                      IPARTIC, LC_NSTK, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYER_WFS, TAYLOR_ORDER, & ! Numbers (Basic)
                      ...
                      LP_TRANS_ATMOS_FINAL, LP_INITIAL_TRANS,  LP_AVERAGE_SECANT, LP_T_DELT_MUBAR, & ! Input, Linearized Beam param
...
!   4/9/19.  For the Profile-Linearized BOA source term, need additional inputs
...
!   -- (RTS 2/16/21). Use local bookkeeping variable LC_NSTK in addition to NSTOKES
                ...
                IF ( DO_UPWELLING ) THEN
                  CALL UPUSER_PROFILEWF ( &
                    ...
                    DO_LAYER_SCATTERING, DO_MSSTS, FOURIER, IPARTIC, NSTOKES,                     & ! Input flags/numbers (basic)
                    LC_NSTK, NSTREAMS, NLAYERS,                                                   & ! Input flags/numbers (basic)
                    N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS,        & ! Input numbers (basic)
                    ...
                    LP_INITIAL_TRANS, LP_AVERAGE_SECANT, LP_T_DELT_MUBAR, L_T_DELT_DISORDS,          & ! Lin BeamParm/DODTrans
                    ...
                    L_HMULT_1, L_HMULT_2, LP_EMULT_UP, L_UT_HMULT_UU, L_UT_HMULT_UD, LP_UT_EMULT_UP, & ! Lin multipliers
                    L_UPAR_UP_1, LP_UPAR_UP_2, L_ATERM_SAVE, L_BTERM_SAVE, L_WLOWER, NCON, PCON,     & ! Lin PI Solutions
...
!mick fix 1/5/2021 - added NLAYERS to inputs
                ...
                IF ( DO_DNWELLING ) THEN
                  CALL DNUSER_PROFILEWF ( &
                    ...
                    NSTOKES, LC_NSTK, NLAYERS, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS,           & ! Input flags/numbers (basic)
                    ...
                    L_T_DELT_USERM, L_T_UTDN_USERM, LP_INITIAL_TRANS, LP_AVERAGE_SECANT, LP_T_DELT_MUBAR, & ! Lin BeamParm/Trans
                    L_KEIGEN, L_UHOM_DNDN, L_UHOM_DNUP, L_LAYER_TSUP_DN, L_LAYER_TSUP_UTDN,               & ! Lin Homog/Thermal
                    L_HMULT_1, L_HMULT_2, LP_EMULT_DN, L_UT_HMULT_DU, L_UT_HMULT_DD, LP_UT_EMULT_DN,      & ! Lin multipliers
                    L_UPAR_DN_1, LP_UPAR_DN_2, L_ATERM_SAVE, L_BTERM_SAVE, NCON, PCON,                    & ! Lin PI Solutions
...
!mick fix 1/5/2021 - replaced "NV" with "LAYER_TO_VARY"
!   -- (RTS 2/16/21). Use local bookkeeping variables LC_NSTK, and others
                ...
                IF ( DO_INCLUDE_MVOUTPUT ) THEN
                  CALL MIFLUX_PROFILEWF ( &
                    ...
                    DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,                 & ! Input flags
                    IPARTIC, LAYER_TO_VARY, N_LAYER_WFS, LC_NSTK, NSTREAMS, LC_NSTKNSTRM, NLAYERS, & ! Input numbers
                    ...
                    MEANST_DIFFUSE_PROFWF, DNMEANST_DIRECT_PROFWF,                               & ! Lin Output Actinic Fluxes
                    FLUX_DIFFUSE_PROFWF, DNFLUX_DIRECT_PROFWF )                                    ! Lin Output Regular Fluxes
                ENDIF
...
!  regular
              ...
              IF ( STATUS_SUB .NE. VLIDORT_SUCCESS ) THEN
                write(CF,'(I2)')FOURIER
                TRACE_2 = 'Error return from VLIDORT_CBEAM_SOLUTION, '//'Called in VLIDORT_LPS_FOURIER, Fourier # '//CF
                STATUS = VLIDORT_SERIOUS ; RETURN
              ENDIF
...
!  Linearized
              ...
              IF ( DO_ATMOS_LINEARIZATION ) THEN
                ...
                CALL VLIDORT_LP_CBEAM_SOLUTION ( &
                  DO_PLANE_PARALLEL, DO_RTSOL_VARY, LAYER, FOURIER, IBEAM, LC_NSTK, & ! Input flags/numbers
                  NSTREAMS, NMOMENTS, NPARAMS_VARY, NSTREAMS_2, LC_NSTKNSTRM,       & ! Input numbers
...
!  Classical
                ...
                IF ( DO_CLASSICAL_SOLUTION ) THEN
                  ...
                  IF ( DO_ATMOS_LINEARIZATION ) THEN
                    CALL VLIDORT_LP_CUSER_SOLUTION ( &
...
!   -- (RTS 2/16/21). Use local bookkeeping variables LC_NSTK, and others
                     ...
                     CALL LP_BVP_SOLUTION_MASTER ( &
                      ...
                      DO_WATER_LEAVING, DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS,          & ! Flags
                      DO_CLASSICAL_SOLUTION, DO_PLANE_PARALLEL, DO_LAYER_SCATTERING, FOURIER,      & ! Flags/Fourier
                      IBEAM, LC_NSTK, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYER_WFS, TAYLOR_ORDER, & ! Numbers (Basic)
                      NSTREAMS_2, LC_NSTKNSTRM, LC_NSTKNSTRM_2, LC_NTT, LC_NSUBD, LC_NSUPD,        & ! Numbers (derived)
                      MUELLER_INDEX, K_REAL, K_COMPLEX, SURFACE_FACTOR, QUAD_STRMWTS,              & ! Bookkeeping
                      DELTAU_SLANT, DELTAU_VERT, L_DELTAU_VERT, RF_DIRECT_BEAM, SL_QUADTERM,       & ! Optical and direct beam
                      BEAM_CUTOFF, INITIAL_TRANS, AVERAGE_SECANT, T_DELT_MUBAR,                    & ! Input, Beam parameterization
...
!   -- (RTS 2/16/21). Use local bookkeeping variables LC_NSTK, and others
                  ...
                  ELSE
                    ...
                    CALL LP_BVPTEL_SOLUTION_MASTER ( &
                      ...
                      TAYLOR_ORDER, LC_NSTK, NSTREAMS, NLAYERS, LAYER_TO_VARY, N_LAYER_WFS,          & ! Basic Control Numbers
...
!    -- (RTS 2/16/21). Use local bookkeeping variable LC_NSTK, as well as NSTOKES
                  ...
                  IF ( DO_UPWELLING ) THEN
                    CALL UPUSER_PROFILEWF ( &
                      ...
                      DO_CLASSICAL_SOLUTION, DO_MSMODE_VLIDORT,    DO_INCLUDE_SURFACE,              & ! Input flags (sources)
                      DO_LAMBERTIAN_SURFACE, DO_DBCORRECTION, DO_INCLUDE_DIRECTRF, DO_INCLUDE_DIRECTSL, & ! Input flags (Surface)
                      DO_LAYER_SCATTERING, DO_MSSTS, FOURIER, IBEAM, NSTOKES,                       & ! Input flags/numbers (basic)
                      LC_NSTK, NSTREAMS, NLAYERS,                                                   & ! Input flags/numbers (basic)
                      N_PPSTREAMS, PPSTREAM_MASK, N_USER_LEVELS, LAYER_TO_VARY, N_LAYER_WFS,        & ! Input numbers (basic)
                      LEVELMASK_UP, PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,   & ! Input Level output control
                      TAYLOR_ORDER, PARTAU_VERT, DELTAU_VERT, DELTAU_SLANT, L_DELTAU_VERT,          & ! Input Taylor/Optical
                      ...
                      T_DELT_USERM, T_UTUP_USERM, SURFACE_FACTOR, ALBEDO, BRDF_F, USER_BRDF_F,      & ! Input UTrans/Surface.
                      RF_USER_DIRECT_BEAM, SL_USERTERM, LP_TRANS_ATMOS_FINAL, CUMSOURCE_UP,         & ! Input surf rads/Cumsource
                      K_REAL, K_COMPLEX, SOLA_XPOS, SOLB_XNEG, T_DELT_EIGEN, LCON, MCON,            & ! Input Homogeneous Solutions
                      ATERM_SAVE, BTERM_SAVE, GAMMA_M, GAMMA_P, PMULT_UU, PMULT_UD, SIGMA_P,        & ! Input Green's function vars
                      T_WLOWER, UHOM_UPDN, UHOM_UPUP, UPAR_UP_1, UPAR_UP_2, HMULT_1, HMULT_2,       & ! Input Thermal/Homog
                      EMULT_UP, UT_HMULT_UU, UT_HMULT_UD, UT_EMULT_UP, UT_PMULT_UU, UT_PMULT_UD,    & ! Input multipliers  
...
!   -- (RTS 2/16/21). Use local bookkeeping variables LC_NSTK, and others
                  ...
                  IF ( DO_INCLUDE_MVOUTPUT ) THEN
                    CALL MIFLUX_PROFILEWF ( &
                      ...
                      DO_SOLAR_SOURCES, DO_INCLUDE_THERMEMISS, DO_THERMAL_TRANSONLY,               & ! Input flags
                      IBEAM, LAYER_TO_VARY, N_LAYER_WFS, LC_NSTK, NSTREAMS, LC_NSTKNSTRM, NLAYERS, & ! Input numbers
                      N_USER_LEVELS, N_DIRECTIONS, WHICH_DIRECTIONS, LEVELMASK_UP, LEVELMASK_DN,   & ! Level/Dir output control
                      PARTLAYERS_OUTFLAG, PARTLAYERS_OUTINDEX, PARTLAYERS_LAYERIDX,                & ! Partial layer output control
                      FLUX_MULT, FLUX_FACTOR, FLUXVEC, LOCAL_CSZA, TAYLOR_ORDER,                   & ! Input Flux/Angles/Taylor
                      QUAD_STREAMS, QUAD_WEIGHTS, QUAD_STRMWTS, PARTAU_VERT, L_DELTAU_VERT,        & ! Input quadrature/Optical
                      BEAM_CUTOFF, INITIAL_TRANS, AVERAGE_SECANT, T_DELT_MUBAR, T_UTDN_MUBAR,      & ! Beam param
                      T_DELT_DISORDS, T_UTDN_DISORDS, T_UTUP_DISORDS, K_REAL, K_COMPLEX,           & ! DODTrans/Bookkeep
                      SOLA_XPOS, SOLB_XNEG, T_DELT_EIGEN, T_UTDN_EIGEN, T_UTUP_EIGEN,              & ! Homog solution
                      ATERM_SAVE, BTERM_SAVE, GAMMA_M, GAMMA_P, UT_GMULT_UP, UT_GMULT_DN,          & ! Green's function solution
                      WUPPER, LCON, MCON, T_WLOWER, T_WUPPER, BOA_THTONLY_SOURCE,                  & ! Solar/BVP/Thermal
