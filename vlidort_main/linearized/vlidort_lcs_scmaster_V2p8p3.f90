...
! #                                                             #
! #          SPHERICITY_CORRECTION_LCS_MASTER (master, public)  #
! #          MS2pt_LCS_SPHERCORR_V1                             #
...
!    -- Use single master routine to call required LCS correction (new)

!  HERE IS THE VLIDORT IMPLEMENTATION
!  ==================================

!  This is the LCS linearization of the sphericity correction
...
!  11/28/22. Upgrade to Version 2.8.3.
!    -- Geometry conversion subroutines are public (needed elsewhere)

      USE VLIDORT_SPHERCORR_ROUTINES_m, Only : &
            BOATOA_2point_conversion, BOATOA_3point_conversion, BOATOA_Mpoint_conversion

!  11/28/22. Upgrade to Version 2.8.3.
!    -- NEW LCS_master routine is public

public  ::  SPHERICITY_CORRECTION_LCS_MASTER

!  All three MSType routines are private

private :: MS2pt_LCS_SPHERCORR_V1, &
           MS3pt_LCS_SPHERCORR_V1, &
           MSMpt_LCS_SPHERCORR_V1

contains

SUBROUTINE SPHERICITY_CORRECTION_LCS_MASTER  ( do_debug_input, & 
...
!  Column Jacobian output with explanations
      ...
      DOUBLE PRECISION, INTENT(OUT) :: FO_COLUMNWF_BOAGEOM   ( MAX_ATMOSWFS, 4 ) ! 1. First-order (SS/DB) LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(OUT) :: MS_COLUMNWF_BOAGEOM   ( MAX_ATMOSWFS, 4 ) ! 2. MS LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(OUT) :: MS_COLUMNWF_SPHERCORR ( MAX_ATMOSWFS, 4 ) ! 3. MS LC Jac with 2-point corr
      DOUBLE PRECISION, INTENT(OUT) :: COLUMNWF_BOAGEOM      ( MAX_ATMOSWFS, 4 ) ! 4. Full LC Jac for BOA geo.     4 = 1 + 2
      DOUBLE PRECISION, INTENT(OUT) :: COLUMNWF_SPHERCORR    ( MAX_ATMOSWFS, 4 ) ! 5. Full LC Jac with MS2pt corr. 5 = 1 + 3

!  Surface Jacobian output with explanations

      DOUBLE PRECISION, INTENT(OUT) :: FO_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 1. First-order (SS/DB) LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(OUT) :: MS_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 2. MS LS Jac for BOA geo
      DOUBLE PRECISION, INTENT(OUT) :: MS_SURFACEWF_SPHERCORR ( MAX_SURFACEWFS, 4 ) ! 3. MS LS Jac with 2-point corr
      DOUBLE PRECISION, INTENT(OUT) :: SURFACEWF_BOAGEOM      ( MAX_SURFACEWFS, 4 ) ! 4. Full LS Jac for BOA geo.     4 = 1 + 2
      DOUBLE PRECISION, INTENT(OUT) :: SURFACEWF_SPHERCORR    ( MAX_SURFACEWFS, 4 ) ! 5. Full LS Jac with MS2pt corr. 5 = 1 + 3
...
!  Call to dedicated LCS_sphericity correction routine (either 2-point, 3-point or multi-point)
...
!  Exception handling
      ...
      IF ( Fail ) THEN
        Main_message = 'Failure from Call to MS'//MStype//'pt_LCS_SPHERICITY_CORRECTION'
      endif

!  return

      return
END SUBROUTINE SPHERICITY_CORRECTION_LCS_MASTER
...
!  Output
!  ------
!mick fix 12/20/2022 - changed intent of main outputs to "inout"
...
!  Standard output with explanations
      ...
      DOUBLE PRECISION, INTENT(INOUT) :: STOKES_BOAGEOM(4)      ! 4. Full STOKES for BOA geo.          4 = 1 + 2.
      DOUBLE PRECISION, INTENT(INOUT) :: STOKES_SPHERCORR(4)    ! 5. Full STOKES with Sphericity corr. 5 = 1 + 3.

!  Column Jacobian output with explanations

      DOUBLE PRECISION, INTENT(INOUT) :: FO_COLUMNWF_BOAGEOM   ( MAX_ATMOSWFS, 4 ) ! 1. First-order (SS/DB) LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(INOUT) :: MS_COLUMNWF_BOAGEOM   ( MAX_ATMOSWFS, 4 ) ! 2. MS LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(INOUT) :: MS_COLUMNWF_SPHERCORR ( MAX_ATMOSWFS, 4 ) ! 3. MS LC Jac with 2-point corr
      DOUBLE PRECISION, INTENT(INOUT) :: COLUMNWF_BOAGEOM      ( MAX_ATMOSWFS, 4 ) ! 4. Full LC Jac for BOA geo.     4 = 1 + 2
      DOUBLE PRECISION, INTENT(INOUT) :: COLUMNWF_SPHERCORR    ( MAX_ATMOSWFS, 4 ) ! 5. Full LC Jac with MS2pt corr. 5 = 1 + 3

!  Surface Jacobian output with explanations

      DOUBLE PRECISION, INTENT(INOUT) :: FO_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 1. First-order (SS/DB) LC Jac for BOA geo
      DOUBLE PRECISION, INTENT(INOUT) :: MS_SURFACEWF_BOAGEOM   ( MAX_SURFACEWFS, 4 ) ! 2. MS LS Jac for BOA geo
      DOUBLE PRECISION, INTENT(INOUT) :: MS_SURFACEWF_SPHERCORR ( MAX_SURFACEWFS, 4 ) ! 3. MS LS Jac with 2-point corr
      DOUBLE PRECISION, INTENT(INOUT) :: SURFACEWF_BOAGEOM      ( MAX_SURFACEWFS, 4 ) ! 4. Full LS Jac for BOA geo.     4 = 1 + 2
      DOUBLE PRECISION, INTENT(INOUT) :: SURFACEWF_SPHERCORR    ( MAX_SURFACEWFS, 4 ) ! 5. Full LS Jac with MS2pt corr. 5 = 1 + 3
