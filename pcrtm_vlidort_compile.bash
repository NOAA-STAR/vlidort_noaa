...
#  if [ $(diff -q vlidort_$1_test/makefile makefile | wc -l) != "0" ] ; then
  if [ $(diff -q PCRTM_VLIDORT_Main/makefile makefile | wc -l) != "0" ] ; then

    echo 'makefile has changed - copying new version up to script directory ...'
#    cp vlidort_$1_test/makefile .
     echo 'cp PCRTM_VLIDORT_Main/makefile '
    cp PCRTM_VLIDORT_Main/makefile .
...
for ((i=1 ; i<=15 ; i++)) ; do
  ...
  if [ $i -le 12 ] ; then
    ...
    if [ "${test[i]}" = "1" ] ; then
     ...
     if [ $i -eq 1 ]; then
#        fn_prefix="v2p8p5_"    # xiong, 5/23/2022
        fn_prefix="pcrtm_"  # Xiong, 7/5/2022
      fi

      #do selected test
      exec="${fn_prefix}${testname[i]}_simulator${fn_suffix}"
      echo $exec $2  $3
      echo
#      echo "making ${testname[i]}_simulator ..."
      echo
      make $exec FC=$2 $3
      echo
       #exit 0
      #echo "running ${testname[i]}_simulator ..."
      echo
#      ./$exec 
      echo
    fi
