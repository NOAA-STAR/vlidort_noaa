...
!         precision ( Interpolate_Lin_fOQ_BS1, Interpolate_Lin_fOQ_BS2,
!         Interpolate_Lin_fOQ_BS3, Interpolate_Lin_fOQ_BSF, Lin_WhiteCap_Reflectance)
...
!  Basic quantities for ocean reflectance
   ...
   real(fpk) :: Const0, Const1, Const_Iso, denom, mu0, Rwprime, f
   real(fpk) :: eta, help1
   real(fpk) :: Albedo
...
!  Linearization variables
   ...
   real(fpk) :: dC_eta, dC_help1, dC_f
   real(fpk) :: dC_Const_Iso, dC_Rwprime

   real(fpk) :: dW_Const_Iso, dW_Const0, dW_Const1 
...
!  rough surface transmittance help variables
   ...
   real(fpk) :: dW_Trans_solar(Maxszas)
...
!  First make the basic call. This will then need to be multiplied by an foQ factor
   ...
   call  Lin_Ocean_Reflectance_Basic &
     ( AOS_MODEL, ABTOT_CONTROL, Wavelength, PigmentConc, noWL, &
       Ocean_Reflec_Basic, eta, dC_Ocean_Reflec_Basic, dC_eta )
...
!  Basic quantities. Flat Surface value of TRANS_SOLAR = 1.0
      ...
      mu0 = SZA_cosines(J)
      f    = 0.6279_fpk - (0.2227_fpk*eta) - (0.0513_fpk*eta*eta) + (0.2465_fpk*eta -0.3119_fpk ) * mu0
...
!  C-derivatives
      ...
      dC_f = ( - 0.2227_fpk - 0.1026_fpk*eta + 0.2465_fpk* mu0 ) * dC_eta
...
!    -- 1/31/21, Version 2.8.3. include azimuth dependent output WLeaving_SVA, dC_WLeaving_SVA
!    -- 1/31/21, Version 2.8.3. include Fourier dependent output WLeaving_SVF, dC_WLeaving_SVF
!    -- 1/31/21, Version 2.8.3. include Fourier dependent output WLeaving_SDF, dC_WLeaving_SDF
...
!    -- 1/31/21, Version 2.8.3. include azimuth dependent output dW_WLeaving_SVA
!    -- 1/31/21, Version 2.8.3. include Fourier dependent outputs dW_WLeaving_SVF, dW_WLeaving_SDF
...
subroutine Lin_Ocean_Reflectance_Basic &
       ( AOS_MODEL, ABTOT_CONTROL, Wavelength, PigmentConc, noWL, &
         Ocean_Reflec, eta_out, dC_Ocean_Reflec, dC_eta_out )
...
! Absorption/scattering terms, and parameters for calculation of f
      ...
      real(fpk) :: vas_chl_a,vas_chl_b,lee_chl_a,lee_chl_b,a_440,Loga_440,helpw
...
!  8/4/22. Additional Variables for the Chlorophyll absorption
      ...
      real(fpk) :: x, xsq, y1, y2, g1, g2, h1, h2, k1, k2, acon, bcon, ccon, dcon
      real(fpk) :: d_vas_chl_a,d_vas_chl_b,d_lee_chl_a,d_lee_chl_b

!  derivative variables

      real(fpk) :: da_tot,db_tot,da_ph,da_cdom,dv,dbp,dbbp,deta,dxc,dz,xc,z,dR2
...
! exponent for spectral dependence of scattering
      ...
      xc=log10(C) ; dxc = 1.0_fpk/C/log(10.0_fpk) 
      if (C .le. 2.0_fpk) then
        v=0.5_fpk*(xc-0.3_fpk) ; dv = 0.5_fpk*dxc
      endif
      ...
      bbp  = 0.002_fpk+0.01_fpk*(0.5_fpk-0.25_fpk*xc)*z
      dbbp = -0.0025_fpk*dxc*z+0.01_fpk*(0.5_fpk-0.25_fpk*xc)*dz
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
      ...
      eta_out=b_wat/b_tot      ; dC_eta_out = -eta_out * db_tot / b_tot
...
!  Single precision in the original
   ...
   integer   :: iwl, iref
   real(fpk) :: Wlb, DWlb, WLP, Ref(39), wspd, wl, Ref_i, Rwc, DRwc
...
!  Help variables
   ...
   real(fpk) :: dtr, fw1, fw2, fs1, fs2, cszas(Maxszas), yspline(6), bbas(6), cbas(6), dbas(6)
   real(fpk) :: Wave, csza, C, LogC, lam, sun, pigc, fval, Logder, foQ_suns(6), dC_foQ_suns(6)
...
!      SUBROUTINE spline(x,y,n,yp1,ypn,y2)
   ...
   C = PigmentConc ; LogC = log(C)
   do j = 1, 6
      do k = 1, 6
         yspline(k) = fw1*foQ_averaged(w1,j,k) + fw2*foQ_averaged(w2,j,k) 
      enddo
      Call BSPLINE2(6,6,LogPigs,yspline,bbas,cbas,dbas)
      Call DSEVAL2(6,6,LogC,LogPigs,yspline,bbas,cbas,dbas,foQ_suns(j),Logder) ; dC_foQ_suns(j) = LogDer / C
   enddo
...
!  Solar angles
   ...
   do n = 1, nszas
      ...
      dC_FoQ_Int_1(n) = fs1 * dC_foQ_suns(s1) + fs2 * dC_foQ_suns(s2)
...
!  Help variables
   ...
   real(fpk) :: Wave, C, LogC, cs, cd, lam, sun, pigc, fval, foQ_1, foQ_2, Logder
   real(fpk) :: foQ_suns(6), foQ_nads(6,17), dC_foQ_suns(6), dC_foQ_nads(6,17)
   real(fpk) :: dtr, local_sine, local_cos, incident_angle, refrac_R_sp
...
!      SUBROUTINE spline(x,y,n,yp1,ypn,y2)
   ...
   do j = 1, 6
      ...
      do m = 1, 17
         ...
         enddo
         Call BSPLINE2(6,6,LogPigs,yspline,bbas,cbas,dbas)
         Call DSEVAL2(6,6,LogC,LogPigs,yspline,bbas,cbas,dbas,foQ_nads(j,m),Logder) ; dC_foQ_nads(j,m) = LogDer / C
      enddo
   enddo
...
!            d1 = 5 ; d2 = d1 + 1 ; fd1 = 0.0_fpk ; fd2 = 1.0_fpk   !  Wrong offset, produced glitches
         ...
         dC_FoQ_Int_SD(n,i) = fs1 * foQ_1 + fs2 * foQ_2
      enddo
...
!            d1 = 5 ; d2 = d1 + 1 ; fd1 = 0.0_fpk ; fd2 = 1.0_fpk   !  Wrong offset, produced glitches
         ...
         foQ_1 = fd1*dC_foQ_nads(s1,d1) + fd2*dC_foQ_nads(s1,d2)
         foQ_2 = fd1*dC_foQ_nads(s2,d1) + fd2*dC_foQ_nads(s2,d2)
         dC_FoQ_Int_SV(n,i) = fs1 * foQ_1 + fs2 * foQ_2
      enddo
...
!    1/31/21, Version 2.8.3. Some new help variables
   ...
   real(fpk) :: Wave, C, LogC, cd, lam, sun, pigc, fval, Logder

   real(fpk) :: foQ_suns(6), foQ_nads(6,17), foQ_nadazs(6,17,13)
   real(fpk) :: dC_foQ_suns(6), dC_foQ_nads(6,17), dC_foQ_nadazs(6,17,13)
   ...
   do j = 1, 6
      ...
      do m = 1, 17
         ...
         do z = 1, 13
            ...
            enddo
            Call BSPLINE2(6,6,LogPigs,yspline,bbas,cbas,dbas)
            Call DSEVAL2(6,6,LogC,LogPigs,yspline,bbas,cbas,dbas,foQ_nadazs(j,m,z),Logder) ; dC_foQ_nadazs(j,m,z) = LogDer / C
         enddo
      enddo
   enddo
...
!   -- 1/31/21, Version 2.8.3. Azimuth section is new
      ...
      do i = 1, nvzas
         ...
         do j = 1, nazms
            ...
            foQ_1 = fd1*foQ_s1d1 + fd2*foQ_s1d2 
            foQ_2 = fd1*foQ_s2d1 + fd2*foQ_s2d2 
            dC_FoQ_Int_SVA(n,i,j) = fs1 * foQ_1 + fs2 * foQ_2
         enddo
      enddo
...
!    1/31/21, Version 2.8.3. Some new help variables
   ...
   real(fpk) :: fw1, fw2, fs1, fs2, fd1, fd2, fz1, fz2, fq1, fq2, yspline(6), bbas(6), cbas(6), dbas(6)
   real(fpk) :: Wave, LogC, C, cd, lam, sun, pigc, fval, foQ_nadazs(6,17,13), dC_foQ_nadazs(6,17,13)
   real(fpk) :: dtr, local_sine, local_cos, incident_angle, refrac_R_sp, foQ_1, foQ_2, LogDer
...
!   -- 1/31/21, Version 2.8.3. Azimuth section is new
      ...
      do i = 1, nstreams
         ...
         do j = 1, naqhalf
            ...
            foQ_1 = fq1*foQ_s1q1 + fq2*foQ_s1q2 
            foQ_2 = fq1*foQ_s2q1 + fq2*foQ_s2q2 
            dC_FoQ_Int_SDQ(n,i,j) = fs1 * foQ_1 + fs2 * foQ_2
         enddo
      enddo
...
!   -- 1/31/21, Version 2.8.3. Azimuth section is new
      ...
      do i = 1, nvzas
         ...
         do j = 1, naqhalf
            ...
            foQ_1 = fd1*foQ_s1d1 + fd2*foQ_s1d2 
            foQ_2 = fd1*foQ_s2d1 + fd2*foQ_s2d2 
            dC_FoQ_Int_SVQ(n,i,j) = fs1 * foQ_1 + fs2 * foQ_2
         enddo
      enddo
