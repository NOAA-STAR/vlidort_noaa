...
!    ==> Explicit treatment of doublet goemetry
      ...
      USE VLIDORT_PARS_m, Only : MAX_USER_RELAZMS, MAX_USER_STREAMS, MAX_USER_OBSGEOMS, &
                                 ...
                                 MAXMOMENTS, VLIDORT_SUCCESS, VLIDORT_SERIOUS
      ...
      USE vsleave_lin_sup_routines_2_m , only : Linearized_WaterLeaving_2EE
...
!  2/28/2., Version 3.8.3. Azimuth dependence in the water-leaving radiance
...
!              2           0.868           685.2        9.55
      ...
      data FL_DataGAUSSIANS(1,1) / 1.445_fpk /
      data FL_DataGAUSSIANS(2,1) / 736.8_fpk /
      data FL_DataGAUSSIANS(3,1) / 21.2_fpk  /
      data FL_DataGAUSSIANS(1,2) / 0.868_fpk /
      data FL_DataGAUSSIANS(2,2) / 685.2_fpk /
      data FL_DataGAUSSIANS(3,2) / 9.55_fpk  /
...
!  Help variables
      ...
      INTEGER   :: IB, K, nslpars, I, M, Q, UM, NAZ, LUM, LUA, NMOMENTS, nazmquads, localm
...
!  For each Solar zenith angle
 ...
 !  Get the F_755 data from the subroutine
          ...
          CALL get_fluorescence_755 ( FL_Latitude, FL_Longitude, FL_Epoch, BEAM_SZAS(IB), FluoFile, Fs755(IB) )
...
!  Compute double Gaussian sums
          ...
          do k = 1, 2
             ...
             Gauss = zero
             if ( arg.lt.88.0_fpk ) gauss = ampli * dexp ( - arg )
             FsSum = FsSum + Gauss
          enddo
...
!mick mod 9/19/2017 - exception handling upgraded for Version 2.8
            ...
            if ( fail ) then 
              MESSAGES(NMESSAGES+1) = 'Fatal - WaterLeaving_2E failed in VSLEAVE_LIN_MAINMASTER'
              MESSAGES(NMESSAGES+2) = Trim(message)
              NMESSAGES = NMESSAGES + 2 ; STATUS = VLIDORT_SERIOUS ; GO TO 899
            endif
...
!mick mod 9/19/2017 - exception handling upgraded for Version 2.8
           ...
           if ( fail ) then
             MESSAGES(NMESSAGES+1) = 'Fatal - Lin_WaterLeaving_2EE failed in VSLEAVE_LIN_MAINMASTER'
             MESSAGES(NMESSAGES+2) = Trim(message)
             NMESSAGES = NMESSAGES + 2 ; STATUS = VLIDORT_SERIOUS ; GO TO 899
           endif
...
!    ==> Introduce doublet and observational geometry output settings
         ...
         do ib = 1, nbeams
            if ( do_user_obsgeoms ) then
              ...
              VSLEAVE_Sup_Out%SL_USER_SLTERM_F_0  (0:localm,1,lum,ib) = WLeaving_SVF(0:localm,ib,ib)
            else if ( do_doublet_geometry ) then
            ...
            do i = 1, nstreams
              VSLEAVE_Sup_Out%SL_SLTERM_F_0(0:localm,1,i,ib) = WLeaving_SDF(0:localm,ib,i)
            enddo
         enddo
...
!   Pigment
         ...
         if ( DO_CHLORCONC_WF ) then
           ...
           do ib = 1, nbeams
             if ( do_user_obsgeoms ) then
               VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,lum,lua,ib)      = dC_WLeaving_SVA(ib,ib,ib)
               VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,lum,ib) = dC_WLeaving_SVF(0:localm,ib,ib)
             else if ( do_doublet_geometry ) then
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,um,lua,ib)      = dC_WLeaving_SVA(ib,um,um)
                 VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,um,ib) = dC_WLeaving_SVF(0:localm,ib,um)
               enddo
             else
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,um,1:naz,ib)    = dC_WLeaving_SVA(ib,um,1:naz)
                 VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,um,ib) = dC_WLeaving_SVF(0:localm,ib,um)
               enddo
             endif
             do i = 1, nstreams
               VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0(Q,0:localm,1,i,ib) = dC_WLeaving_SDF(0:localm,ib,i)
             enddo
           enddo
         endif
...
!   Windspeed
         ...
         if ( DO_WINDSPEED_WF ) then
           ...
           do ib = 1, nbeams
             if ( do_user_obsgeoms ) then
               VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,lum,lua,ib)      = dW_WLeaving_SVA(ib,ib,ib)
               VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,lum,ib) = dW_WLeaving_SVF(0:localm,ib,ib)
             else if ( do_doublet_geometry ) then
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,um,lua,ib)      = dW_WLeaving_SVA(ib,um,um)
                 VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,um,ib) = dW_WLeaving_SVF(0:localm,ib,um)
               enddo
             else
               do um = 1, n_user_streams
                 VSLEAVE_LinSup_Out%SL_LS_SLTERM_USERANGLES(Q,1,um,1:naz,ib)    = dW_WLeaving_SVA(ib,um,1:naz)
                 VSLEAVE_LinSup_Out%SL_LS_USER_SLTERM_F_0  (Q,0:localm,1,um,ib) = dW_WLeaving_SVF(0:localm,ib,um)
               enddo
             endif
             do i = 1, nstreams
               VSLEAVE_LinSup_Out%SL_LS_SLTERM_F_0(Q,0:localm,1,i,ib) = dW_WLeaving_SDF(0:localm,ib,i)
             enddo
           enddo
         endif
