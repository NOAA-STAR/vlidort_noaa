...
! #      BSPLINE2, SEVAL2, DSEVAL2   Public Bspline software    #
! #                                  (Adapted by R. Spurr 2017) #
! #                                  (Converted to DP Aug 2022) #
! #                                                             #
! ###############################################################
...
!  Threshold for Newton's Method
      ...
      REAL(FPK), PARAMETER :: QEPS = 1.0E-13_FPK
...
! everywhere less than 1.2 * 10^7. 
      ...
      z = abs(x) ; xin = x
      t = 1.0_fpk/(1.0_fpk + 0.5_fpk*z)
      x = t*dexp(-z*z-1.26551223_fpk + t*(1.00002368_fpk + t*(.37409196_fpk + &
              t*(.09678418_fpk + t*(-.18628806_fpk + t*(.27886807_fpk + t* &
              (-1.13520398_fpk + t*(1.48851587_fpk + t*(-.82215223_fpk + t* &
              .17087277_fpk)))))))))
      if ( xin .lt. 0.0_fpk ) x = 2.0_fpk - x
...
!
      ...
      SUBROUTINE SEVAL2 (NMAX,N,U,X,Y,B,C,D,VAL)
...
!mick mod 8/4/2022 - converted to double precision (now "SEVAL2")
...
!  Arguments
      ...
      REAL(fpk), intent(in) :: B(NMAX),C(NMAX),D(NMAX),X(NMAX),Y(NMAX),U
      REAL(fpk), intent(out):: VAL
...
!     SPLINE EVALUATION
   ...
   30 DX  = U-X(I)
      ...
      RETURN
      END SUBROUTINE SEVAL2

!

      SUBROUTINE DSEVAL2 (NMAX,N,U,X,Y,B,C,D,VAL,DVAL)
...
!mick mod 8/4/2022 - converted to double precision (now "DSEVAL2")
...
!     SPLINE EVALUATION
   ...
   30 CONTINUE
      DX  = U-X(I)
      VAL  = Y(I)+DX*(B(I)+DX*(C(I)+DX*D(I)))
      DVAL = B(I) + DX*(2.0_fpk*C(I)+3.0_fpk*DX*D(I))

      RETURN
      END SUBROUTINE DSEVAL2

!

      SUBROUTINE BSPLINE2 (NMAX,N,X,Y,B,C,D)
...
!mick mod 8/4/2022 - converted to double precision (now "BSPLINE2")
...
!  Arguments
      ...
      integer  , intent(in)  :: NMAX,N
      REAL(fpk), intent(in)  :: X(NMAX),Y(NMAX)
      REAL(fpk), intent(out) :: B(NMAX),C(NMAX),D(NMAX)
...
!     COEFFICIENTS OF 3RD DEGREE POLYNOMIAL
      ...
      B(N) = (Y(N)-Y(NM1))/D(NM1)+D(NM1)*(C(NM1)+2.0_fpk*C(N))
      DO 40 I = 1,NM1
        B(I) = (Y(I+1)-Y(I))/D(I)-D(I)*(C(I+1)+2.0_fpk*C(I))
        D(I) = (C(I+1)-C(I))/D(I)
        C(I) = 3.0_fpk*C(I)
   40 CONTINUE
      C(N) = 3.0_fpk*C(N)
