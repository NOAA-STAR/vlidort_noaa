...
!  Flags for use of observational geometry and doublet geometry. Also, sunlight flag.
!   -- 7/24/22. renamed , added doublet flag
 
   LOGICAL, INTENT(IN)   :: do_Obsgeom, do_Doublet, Sunlight
...
!   INTEGER  , PARAMETER :: Max_OutAngles = 2000                   ! quadrature
!   INTEGER  , PARAMETER :: Max_OutAngles = 1000                   ! quadrature
...
!  7/24/22. In each rotation call, introduce doublet flag and offset. Rename carefully
...
!  7/24/22. Add the doublet flag and offset, rename carefully
...
!  7/24/22. Add Doublet flag
   ...
   write(*,*) 'do_Obsgeom   = ',do_Obsgeom
   write(*,*) 'do_Doublet   = ',do_Doublet
   ...
   write(*,*) 'n_vzas       = ',n_vzas
   write(*,*) 'n_azms       = ',n_azms
   if ( do_ObsGeom ) write(*,*) 'n_geoms      = ',n_geoms
   if ( do_Doublet ) write(*,*) 'n_geoms      = ',n_geoms

!  7/24/22. Doublet offset array outputted here, as opposed to Lattice offset array

   write(*,*)
   if ( do_Doublet ) then
     do ib=1,n_szas
       write(*,*) 'ib = ',ib,'Doublet_offsets(ib) = ',Doublet_offsets(ib)
     enddo
   else if ( .not. do_Obsgeom ) then
     do ib=1,n_szas
       do um = 1, n_vzas
         write(*,*) 'ib = ',ib,'um = ',um,'Lattice_offsets(ib,um) = ',Lattice_offsets(ib,um)
       enddo
     enddo
   endif
