...
!  7/4/22. Changes to Pre_Master
!      -- Introduce doublet flag and offset array, in line with usual VLIDORT geometrical options
!      -- rename existing offset array to Lattice_offsets
!      -- Introduce doublet arguments into the Pre_Master subroutine
...
!  ---------------
   ...
   integer :: i, j, k, k1, L, irun, nrun, nrun0, Ounit
...
77 format(A,2i3,i5,A,f10.6)
   ...
   Ounit = 200 ; if ( do_Doublet ) Ounit = 400 ; if ( do_Obsgeom ) ounit = 600
   do k = 1, n_geoms
     write(ounit+1,78)k,OutFmatrices_up(k,1:6)
     write(ounit+2,78)k,OutFmatrices_dn(k,1:6)
   enddo
   do L = 0, ncoeffs
      write(ounit+3,78)L,FMatCoeffs(L,1:6)
   enddo
