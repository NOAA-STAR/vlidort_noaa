! modified based on the new delivery from Robert on May 1, 2023, by Shawn Xiong
!
! ###############################################################
! #                                                             #
! #                       VLIDORT_2p8p3                         #
...
!               -- Introduce doublet arguments into the rotation subroutine
...
subroutine vfzmat_Rayleigh &
   ( max_geoms, max_szas, max_vzas, max_azms, dtr,              & ! input  Dimensions (VLIDORT)
     do_upwelling, do_dnwelling, do_ObsGeom, do_Doublet,        & ! input  Flags
     Sunlight, nstokes, n_geoms, n_szas, n_vzas, n_azms,        & ! input  Numbers
...
!  Flags for use of observational geometry and doublet geometry
!   -- 7/24/22. renamed , added doublet flag

   LOGICAL, INTENT(IN)   :: do_Obsgeom, do_Doublet

!  Flags for use of Sunlight option
...
!   REAL    (KIND=dpk) :: mu, P2, beta2
   REAL    (KIND=dpk) :: mu, P2, beta2, P2M2, P2P2, QR6
...
!  7/24/22. Section rearranged to include Doublet geometry option
...
!   5/25/20. Version 2.8.2 Upgrade. Factor half removed
  ...
  if ( do_upwelling ) then
    do V = 1, n_geoms
      mu = COSSCAT_up(v) ; P2 = 1.5_dpk * mu * mu - d_half; P2P2 = QR6 * (1.0_dpk - mu * mu )
...
!        RayFmatrices_up(v,5) = - P2 * RayCoeffs(2,5)        !!!!!!! WRONG
        RayFmatrices_up(v,5) = P2P2 * RayCoeffs(2,5)
  ...
  if ( do_dnwelling ) then
    do V = 1, n_geoms
      mu = COSSCAT_dn(v) ; P2 = 1.5_dpk * mu * mu - d_half; P2M2 = QR6 * (1.0_dpk - mu * mu )
...
!        RayFmatrices_dn(v,5) = - P2 * RayCoeffs(2,5)        !!!!!!! WRONG
        RayFmatrices_dn(v,5) = P2M2 * RayCoeffs(2,5)
...
!  7/24/22. In each case, introduce doublet flag and offset. Rename carefully

   if ( do_upwelling ) then
      Call vfzmat_Rotation &
       ( max_geoms, max_szas, max_vzas, max_azms, vsign, dtr,              & ! Inputs
         do_ObsGeom, do_Doublet, nstokes, n_geoms, n_szas, n_vzas, n_azms, & ! inputs
         Lattice_offsets, Doublet_offsets, szas, vzas, azms, obsgeoms,     & ! Inputs
         C1, S1, C2, S2 )
