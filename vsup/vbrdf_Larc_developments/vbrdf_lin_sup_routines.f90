...
!
      ...
      SUBROUTINE VBRDF_LIN_FOURIER &
         ( DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_USER_STREAMS, DO_SURFEMISS, LAMBERTIAN_FLAG,             & ! Control flags
           DO_HALF_RANGE, DO_FOURIER_USERSUN, M, NSTOKES, NBEAMS, NSTREAMS, N_USER_STREAMS, NSTREAMS_BRDF, & ! Numbers
           NBRDF_HALF, BRDF_NPARS, BRDF_DERIVS, DELFAC, BRDF_COSAZMFAC, BRDF_SINAZMFAC, A_BRDF, BAX_BRDF,  & ! Surface/Azimuth factors
...
!  2/25/22. Version 2.8.5. include flag for calculation of USER_BRDF_F_0.
        ...
        IF ( DO_SOLAR_SOURCES .and. DO_FOURIER_USERSUN  .AND. .NOT. LAMBERTIAN_FLAG ) THEN
          IF ( DO_USER_OBSGEOMS ) THEN
            DO W = 1, BRDF_NPARS
              IF ( BRDF_DERIVS(W) ) THEN
                DO IB = 1, NBEAMS
...
!  Start Replacement code --------------------------------
                DO O1 = 1, NSTOKES
                  DO O2 = 1, NSTOKES
                    ...
                    EMISS(O2) = REFL
                  ENDDO
                  D_L_EMISSIVITY(W,O1,I) = HELP_EMISS * SUM(EMISS(1:NSTOKES))
                ENDDO
!  End Replacement code ---------------------------------
              ENDDO
            ENDIF
...
!  Start Replacement code --------------------------------
                  DO O1 = 1, NSTOKES
                    DO O2 = 1, NSTOKES
                      ...
                      EMISS(O2) = REFL
                    ENDDO
                    D_L_USER_EMISSIVITY(W,O1,UI) = HELP_EMISS * SUM(EMISS(1:NSTOKES))
                  ENDDO
!  End Replacement code ---------------------------------
                ENDDO
              ENDIF
