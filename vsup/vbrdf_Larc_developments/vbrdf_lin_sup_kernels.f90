...
! #          SNOWRTSMODEL_VFUNCTION_PLUS (Analytic SnowBrdf)    #
! #                                                             #
! ###############################################################
...
!     --  First introduced to VLIDORT, 18 November 2020.
      ...
      PUBLIC :: LISPARSE_VFUNCTION_PLUS,           &
                ...
                SNOWRTSMODEL_VFUNCTION_PLUS,       &    ! Version 2.8.5, 2/28/22
...
!
      ...
      SUBROUTINE SNOWRTSMODEL_VFUNCTION_PLUS  &
            ( MAXPARS, NPARS, PARS, DO_DERIV_PARS, NSSQ, XJ, SXJ, XI, SXI, PHI, CPHI, SKPHI, &
              SNOWRTSMODEL_VKERNEL, SNOWRTSMODEL_VDERIVATIVES )
...
!  Subroutine arguments
      ...
      LOGICAL  , intent(in)  :: DO_DERIV_PARS ( MAXPARS )
      REAL(fpk), intent(in)  :: XI, SXI, XJ, SXJ, PHI, CPHI, SKPHI
      REAL(fpk), intent(out) :: SNOWRTSMODEL_VKERNEL ( MAXSTOKES_SQ )
      REAL(fpk), intent(out) :: SNOWRTSMODEL_VDERIVATIVES ( MAXPARS, MAXSTOKES_SQ )
...
!  initialize
      ...
      SNOWRTSMODEL_VKERNEL      = ZERO
      SNOWRTSMODEL_VDERIVATIVES = ZERO
...
!  Final answer
      ...
      SNOWRTSMODEL_VKERNEL(1) = R0 * ( ONE - Alpha * FFunc ) + R1 * alpha - R2

!  derivatives

      if ( do_deriv_pars(1) ) then
         SNOWRTSMODEL_VDERIVATIVES(1,1) = - R0 * FFunc + R1
      endif

!  Done

      RETURN
      END SUBROUTINE SNOWRTSMODEL_VFUNCTION_PLUS
