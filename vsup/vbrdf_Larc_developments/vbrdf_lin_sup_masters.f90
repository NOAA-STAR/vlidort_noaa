...
!  2/25/22. Version 2.8.5. Scalar and vector amplitudes given separately

      LOGICAL ::          DO_KERNEL_FACTOR_WFS  ( MAX_BRDF_KERNELS )
      LOGICAL ::          DO_KERNEL_VFACTOR_WFS ( MAX_BRDF_KERNELS )
...
!  2/25/22. Version 2.8.5. Scalar and vector amplitudes controlled separately

      DO K = 1, MAX_BRDF_KERNELS
        DO_KERNEL_FACTOR_WFS(K)  = .FALSE.
        DO_KERNEL_VFACTOR_WFS(K) = .FALSE.
        DO L = 1, MAX_BRDF_PARAMETERS
...
! 56     FORMAT( A10, I2, F8.4, I2, 4F12.6 ) ! Version 2.8.3
 56     FORMAT( A10, I3, L3, F8.4, F8.4, I3, 4F12.6)
...
!  2/25/22. Version 2.8.5. Introduce additional vector factor flags, Change 57 format

        PAR_STR = 'Kernels, indices, # pars, Factor Jacobian flags, Par Jacobian flags'
        IF (GFINDPAR ( FILUNIT, PREFIX, ERROR, PAR_STR)) THEN
          DO I = 1, N_BRDF_KERNELS

!mick mod 9/19/2017 - replaced integer for # of BRDF kernel parameters with MAX_BRDF_PARAMETERS
            READ (FILUNIT,57,ERR=998) &
               DUM_NAME, DUM_INDEX, DUM_NPARS, DO_KERNEL_FACTOR_WFS(I), DO_KERNEL_VFACTOR_WFS(I),&
...
!    2/25/22. Version 2.8.5, Error if vfactor WF flags set, and separate VFACTORS not set

            IF ( DO_KERNEL_VFACTOR_WFS(I) .and..not.DO_SEPARATE_FACTORS(I) ) THEN
              NM = NM + 1
              MESSAGES(NM) = 'Cannot have VFACTOR Wfs set, if separate factors are not in force'
              ACTIONS(NM)  = 'Check DO_KERNEL_VFACTOR_WFS(I) to false'
              STATUS = VLIDORT_SERIOUS
              NMESSAGES = NM
              GO TO 764
            ENDIF
...
!    2/25/22. Version 2.8.5, Error if vfactor WF flags set, and separate VFACTORS not set
            ...
            IF ( DO_KERNEL_VFACTOR_WFS(I) .and. BRDF_VECTOR_FACTORS(I).eq.ZERO ) THEN
              NM = NM + 1
              MESSAGES(NM) = 'Cannot have VFACTOR Wfs set, if KERNEL_VECTOR_FACTORS is zero'
              ACTIONS(NM)  = 'Check DO_KERNEL_VFACTOR_WFS(I) to false'
              STATUS = VLIDORT_SERIOUS
              NMESSAGES = NM
              GO TO 764
...
!    2/25/22. Version 2.8.5. Add count if DO_KERNEL_VFACTOR_WFS is set

            IF ( DO_KERNEL_FACTOR_WFS(I) ) THEN
              N_KERNEL_FACTOR_WFS = N_KERNEL_FACTOR_WFS  + 1
            ENDIF
            IF ( DO_KERNEL_VFACTOR_WFS(I) ) THEN
              N_KERNEL_FACTOR_WFS = N_KERNEL_FACTOR_WFS  + 1
            ENDIF
...
! 57     FORMAT( A10, I3, I2, 1X, L2, 2X, 4L2 )
 57     FORMAT( A10, I3, I2, 1X, 2L2, 2X, 4L2 )
...
!  2/25/22. Version 2.8.5. Scalar and vector amplitude WF flags given separately

      VBRDF_LinSup_In%BS_DO_KERNEL_FACTOR_WFS   = DO_KERNEL_FACTOR_WFS
      VBRDF_LinSup_In%BS_DO_KERNEL_VFACTOR_WFS  = DO_KERNEL_VFACTOR_WFS
...
!    3/28/22. Version 2.8.5, Introduce Local factors BRDF_DERFAC_LOCAL (for factor derivatives)
      ...
      DOUBLE PRECISION :: PARS ( MAX_BRDF_PARAMETERS ), BRDF_FACTOR_LOCAL(16), BRDF_DERFAC_LOCAL(16)
...
!    2/25/22. Version 2.8.5, Introduce Vector factors, and control for this.
      ...
      DO_KERNEL_VFACTOR_WFS  = VBRDF_LinSup_In%BS_DO_KERNEL_VFACTOR_WFS
...
!    -- 2/25/22. Version 2.8.5. Add DO_KERNEL_VFACTOR_WFS to Bookkeeping
      ...
      IF ( .not. DO_NewCMGLINT .and. (.not. DO_BSAVALUE_WF .and. .not. DO_WSAVALUE_WF ) ) then
         ...
         DO K = 1, N_BRDF_KERNELS
            ...
            IF ( DO_KERNEL_VFACTOR_WFS(K) ) W = W + 1
...
!    -- 3/28/22. Add derivative factor (must be one or zero) 

        BRDF_FACTOR_LOCAL(1:NSTOKESSQ) = BRDF_FACTORS(K)
        BRDF_DERFAC_LOCAL(1:NSTOKESSQ) = ONE
        IF ( DO_SEPARATE_FACTORS(K) .and. NSTOKES.gt.1) THEN
           ...
           BRDF_DERFAC_LOCAL(1) = ZERO
           BRDF_DERFAC_LOCAL(2:NSTOKESSQ) = ONE
        ENDIF
          ...
          IF ( DO_KPARAMS_DERIVS(K) ) THEN
            CALL VBRDF_LIN_MAKER &
             ( LISPARSE_VFUNCTION_PLUS, LISPARSE_VFUNCTION_PLUS, &
               DO_LOCAL_WSA, DO_LOCAL_BSA, DO_WSA_SCALING, DO_HALF_RANGE, DO_FOURIER_USERSUN, & ! New line, Version 2.7
               DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_DOUBLET_GEOMETRY,              &
               DO_DBONLY, LOCAL_MSR, DO_MSRCORR_DBONLY, MSRCORR_ORDER,               &
               DO_USER_STREAMS, DO_SURFACE_EMISSION, n_muquad, n_phiquad,            &
               NSTREAMS_BRDF, NBRDF_HALF, NSTOKESSQ, BRDF_NPARS,                     &
...
!     -- Must be used in conjunction with Lambertian and Ross-Thick kernels
!     -- One Free parameter
!     -- Input index is SNOWRTS_IDX. Output SNOWRTSMODEL_VFUNCTION

        IF ( WHICH_BRDF(K) .EQ. SNOWRTS_IDX ) THEN
          IF ( DO_KPARAMS_DERIVS(K) ) THEN
            CALL VBRDF_LIN_MAKER &
             ( SNOWRTSMODEL_VFUNCTION, SNOWRTSMODEL_VFUNCTION_PLUS, &
               DO_LOCAL_WSA, DO_LOCAL_BSA, DO_WSA_SCALING, DO_HALF_RANGE, DO_FOURIER_USERSUN, & ! New line, Version 2.7
               DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_DOUBLET_GEOMETRY,              &
               DO_DBONLY, LOCAL_MSR, DO_MSRCORR_DBONLY, MSRCORR_ORDER,               &
               DO_USER_STREAMS, DO_SURFACE_EMISSION, n_muquad, n_phiquad,            &
          ...
          ELSE
            CALL VBRDF_MAKER &
             ( SNOWRTSMODEL_VFUNCTION, SNOWRTSMODEL_VFUNCTION,                   &
               DO_LOCAL_WSA, DO_LOCAL_BSA, DO_HALF_RANGE, DO_FOURIER_USERSUN,    & ! New line, Version 2.7
               DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_DOUBLET_GEOMETRY,          &
               DO_DBONLY, LOCAL_MSR, DO_MSRCORR_DBONLY, MSRCORR_ORDER,           &
               DO_USER_STREAMS, DO_SURFACE_EMISSION, N_MUQUAD, N_PHIQUAD,        &
          ...
          IF ( WHICH_BRDF(K) .EQ. NewCMGLINT_IDX ) then
             CALL VBRDF_Lin_NewCM_MAKER &
              ( DO_GlintShadow, DO_FacetIsotropy, WINDSPEED, WINDDIR, Refrac_R, Refrac_I, &
                WC_Reflectance, WC_Lambertian, DWC_Reflectance, DWC_Lambertian, DO_FOURIER_USERSUN, &
          ...
          DO Q = 1, NSTOKESSQ ; O1 = QMASK(Q)
            ...
            DO IB = 1, NBEAMS
              VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,LUM,LUA,IB) = &
                VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,LUM,LUA,IB) + FF * DBKERNEL_BRDFUNC(O1,LUM,LUA,IB)
            ENDDO
          ENDDO
        ELSE IF ( DO_DOUBLET_GEOMETRY ) THEN
          DO Q = 1, NSTOKESSQ ; O1 = QMASK(Q)
            ...
            DO IB = 1, NBEAMS
              DO UM = 1, N_USER_STREAMS
                VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,UM,LUA,IB) = &
                 VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,UM,LUA,IB) + FF * DBKERNEL_BRDFUNC(O1,UM,LUA,IB)
              ENDDO
            ENDDO
          ENDDO
        ELSE
              ...
              DO IB = 1, NBEAMS
                DO UM = 1, N_USER_STREAMS
                  VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,UM,IA,IB) = &
                    VBRDF_Sup_Out%BS_DBOUNCE_BRDFUNC(O1,UM,IA,IB) + FF * DBKERNEL_BRDFUNC(O1,UM,IA,IB)
                ENDDO
              ENDDO
            ENDDO
          ENDDO
...
!  2/25/22. Version 2.8.5. Add the VFACTOR JACOBIANS . 3/28/22. Add the derivative local factors
        ...
        IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
          ...
          IF ( DO_USER_OBSGEOMS ) THEN
            DO Q = 1, NSTOKESSQ ; O1 = QMASK(Q)
              FF = BRDF_DERFAC_LOCAL(Q)
              DO IB = 1, NBEAMS
                VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,LUM,LUA,IB) = FF * DBKERNEL_BRDFUNC(O1,LUM,LUA,IB)
              ENDDO
            ENDDO
          ELSE IF ( DO_DOUBLET_GEOMETRY ) THEN
            DO Q = 1, NSTOKESSQ ; O1 = QMASK(Q)
              ...
              DO IB = 1, NBEAMS
                DO UM = 1, N_USER_STREAMS
                  VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,UM,LUA,IB) = FF * DBKERNEL_BRDFUNC(O1,UM,LUA,IB)
                ENDDO
              ENDDO
            ENDDO
          ELSE
            DO Q = 1, NSTOKESSQ ; O1 = QMASK(Q)
              ...
              DO IA = 1, N_USER_RELAZMS
                DO IB = 1, NBEAMS
                  DO UM = 1, N_USER_STREAMS
                    VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,UM,IA,IB) = FF * DBKERNEL_BRDFUNC(O1,UM,IA,IB)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
        ...
        IF ( DO_KERNEL_VFACTOR_WFS(K) ) THEN
          W = W + 1
          IF ( DO_USER_OBSGEOMS ) THEN
            DO Q = 2, NSTOKESSQ ; O1 = QMASK(Q)
              DO IB = 1, NBEAMS
                VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,LUM,LUA,IB) = DBKERNEL_BRDFUNC(O1,LUM,LUA,IB)
              ENDDO
            ENDDO
          ELSE IF ( DO_DOUBLET_GEOMETRY ) THEN
            DO Q = 2, NSTOKESSQ ; O1 = QMASK(Q)
              DO IB = 1, NBEAMS
                DO UM = 1, N_USER_STREAMS
                  VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,UM,LUA,IB) = DBKERNEL_BRDFUNC(O1,UM,LUA,IB)
                ENDDO
              ENDDO
            ENDDO
          ELSE
            DO Q = 2, NSTOKESSQ ; O1 = QMASK(Q)
              DO IA = 1, N_USER_RELAZMS
                DO IB = 1, NBEAMS
                  DO UM = 1, N_USER_STREAMS
                    VBRDF_LinSup_Out%BS_LS_DBOUNCE_BRDFUNC(W,O1,UM,IA,IB) = DBKERNEL_BRDFUNC(O1,UM,IA,IB)
                  ENDDO
                ENDDO
              ENDDO
            ENDDO
...
!       -- [Was only required for emission, factor now deployed outside this routine]
          ...
          CALL VBRDF_FOURIER &
            ( DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_USER_STREAMS, DO_SURFACE_EMISSION,  & ! Flags
              ...
              DELFAC, BRDF_COSAZMFAC, BRDF_SINAZMFAC, A_BRDF, BAX_BRDF,                  & ! Surface/Azimuth factors
...
!  2/25/22. Version 2.8.5.  Include flag DO_FOURIER_USERSUN for calculation of D_LOCAL_USER_BRDF_F_0. 
!  3/28/22, Kernel FACTOR argument has been dropped. 
!       -- [Was only required for emission, factor now deployed outside this routine]
          ...
          IF ( BRDF_NPARS .GT. 0 ) THEN
            CALL VBRDF_LIN_FOURIER &
             ( DO_SOLAR_SOURCES, DO_USER_OBSGEOMS, DO_USER_STREAMS,                                   & ! Flags
               DO_SURFACE_EMISSION, LAMBERTIAN_KERNEL_FLAG(K), DO_HALF_RANGE, DO_FOURIER_USERSUN,     & ! Flags
               M, NSTOKES, NBEAMS, NSTREAMS, N_USER_STREAMS, NSTREAMS_BRDF, NBRDF_HALF, BRDF_NPARS,   & ! Numbers
               DERIVS, DELFAC, BRDF_COSAZMFAC, BRDF_SINAZMFAC, A_BRDF, BAX_BRDF,                      & ! Surface/Azimuth factors
...
!  ... Basic
            ...
            DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_FACTOR_LOCAL(Q)
              DO I = 1, NSTREAMS
                DO J = 1, NSTREAMS
...
!  ... Linearization w.r.t Kernel Factor (3/28/22, derivative factor is 0 or 1)

            W  = WOFFSET(K)
            IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
              W = W + 1
              DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                DO I = 1, NSTREAMS
                  DO J = 1, NSTREAMS
                    VBRDF_LinSup_Out%BS_LS_BRDF_F(W,M,Q,I,J) = FF * LOCAL_BRDF_F(Q,I,J)
                  ENDDO
                ENDDO
              ENDDO
            ENDIF
...
!  ... Linearization w.r.t Kernel Factor
              ...
              IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
                ...
                DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                  DO I = 1, NSTREAMS
                    DO IB = 1, NBEAMS
                      VBRDF_LinSup_Out%BS_LS_BRDF_F_0(W,M,Q,I,IB) = FF * LOCAL_BRDF_F_0(Q,I,IB)
                    ENDDO
                  ENDDO
                ENDDO
              ENDIF
...
!  ... Linearization w.r.t Kernel Factor
              ...
              IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
                ...
                DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                  DO UM = 1, N_USER_STREAMS
                    DO J = 1, NSTREAMS
                      VBRDF_LinSup_Out%BS_LS_USER_BRDF_F(W,M,Q,UM,J) = FF * LOCAL_USER_BRDF_F(Q,UM,J)
                    ENDDO
                  ENDDO
                ENDDO
              ENDIF

!  ... Linearization w.r.t Kernel VFactor
!        2/25/22. Version 2.8.5. Only if flagged. No re-setting of WOFFSET

              IF ( DO_KERNEL_VFACTOR_WFS(K) ) THEN
                W = W + 1
                DO O1 = 2, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                  DO UM = 1, N_USER_STREAMS
                    DO J = 1, NSTREAMS
                      VBRDF_LinSup_Out%BS_LS_USER_BRDF_F(W,M,Q,UM,J) = FF * LOCAL_USER_BRDF_F(Q,UM,J)
                    ENDDO
...
!  ... Linearization w.r.t Kernel Factor
              ...
              IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
                ...
                IF ( DO_USER_OBSGEOMS ) THEN
                  DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                    DO IB = 1, NBEAMS
                      VBRDF_LinSup_Out%BS_LS_USER_BRDF_F_0(W,M,Q,LUM,IB) = FF * LOCAL_USER_BRDF_F_0(Q,LUM,IB)
                    ENDDO
                  ENDDO
                ELSE
                  DO O1 = 1, NSTOKESSQ ; Q = QMASK(O1) ; FF = BRDF_DERFAC_LOCAL(Q)
                    DO UM = 1, N_USER_STREAMS
                      DO IB = 1, NBEAMS
                        VBRDF_LinSup_Out%BS_LS_USER_BRDF_F_0(W,M,Q,UM,IB) = FF * LOCAL_USER_BRDF_F_0(Q,UM,IB)
                      ENDDO
                    ENDDO
                  ENDDO
                ENDIF
...
!  Linearization w.r.t Scalar Kernel Factor
              ...
              IF ( DO_KERNEL_FACTOR_WFS(K) ) THEN
                ...
                DO Q = 1, NSTOKES
                  FF = BRDF_DERFAC_LOCAL(Q)
                  DO I = 1, NSTREAMS
                    VBRDF_LinSup_Out%BS_LS_EMISSIVITY(W,Q,I) = - FF * LOCAL_EMISSIVITY(Q,I)
                  ENDDO
                  IF ( DO_USER_STREAMS ) THEN
                    DO UI = 1, N_USER_STREAMS
                      VBRDF_LinSup_Out%BS_LS_USER_EMISSIVITY(W,Q,UI) = - FF * LOCAL_USER_EMISSIVITY(Q,UI)
                    ENDDO
                  ENDIF
                ENDDO
              ENDIF

!  Linearization w.r.t Vector Kernel Factor
