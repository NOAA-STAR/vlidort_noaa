...
VLID_TEST_PATH  = PCRTM_VLIDORT_Main
...
#	FFLAGS := $(FFLAGS) -I$(MOD_PATH) -module $(MOD_PATH)
	FFLAGS := $(FFLAGS) -I$(MOD_PATH) -module $(MOD_PATH)  -assume byterecl    # added By Xiong
...
SOURCES_SOLAR = $(FO_SOURCES_Vector) + \
   ...
   $(VFZMAT_SUP_SOURCES)   \
   $(VLID_TEST_PATH)/PCRTM_SW_Aux.f90 \
   $(VLID_TEST_PATH)/GAS_OPT.f90 \
   $(VLID_TEST_PATH)/pcrtm_omps_simulator.f90 
...
SOURCES_THERMAL = $(FO_SOURCES_Vector) + \
   ...
   $(VBRDF_SUP_SOURCES) \
   $(VBRDF_SUP_ACCESS_SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_thermal_tester.f90

SOURCES_BRDF_SELF = $(VBRDF_LINSUP_SOURCES) + \
   $(VLID_TEST_PATH)/V2p8p3_brdf_self_tester.f90
...
SOURCES_SOLAR_LPCS = $(FO_SOURCES_L_Vector) + \
   ...
   $(LPS_SOURCES) \
   $(LCS_SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_solar_lpcs_tester.f90
...
SOURCES_THERMAL_LPCS = $(FO_SOURCES_L_Vector) + \
   ...
   $(LPS_SOURCES) \
   $(LCS_SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_thermal_lpcs_tester.f90
...
SOURCES_BRDFPLUS = $(FO_SOURCES_L_Vector) + \
   ...
   $(VLID_TEST_PATH)/V2p8p3_brdfplus_tester.f90

# Special vector tests

SOURCES_VSLEAVEPLUS = $(FO_SOURCES_L_Vector) + \
   $(SOURCES) \
   $(L_SOURCES) \
   $(LCS_SOURCES) \
   $(VSLEAVE_LINSUP_SOURCES) \
   $(VSLEAVE_LINSUP_ACCESS_SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_vsleaveplus_tester.f90

SOURCES_Siewert2000 = $(FO_SOURCES_Vector) + \
   $(SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_Siewert2000_validation.f90
...
SOURCES_FULL_PLANETARY = $(FO_SOURCES_L_Vector) + \
   ...
   $(LCS_SOURCES)         \
   $(LPS_SOURCES)         \
   $(VLID_TEST_PATH)/V2p8p3_Planetary_tester.f90
...
SOURCES_LWCoupling = $(FO_SOURCES_Vector) + \
   ...
   $(VLID_TEST_PATH)/V2p8p3_LWCoupling_tester.f90

# Parallel tests using OpenMP

SOURCES_SOLAR_OMP = $(FO_SOURCES_Vector) + \
   $(SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_solar_OMP_tester.f90

SOURCES_SOLAR_LPCS_OMP = $(FO_SOURCES_L_Vector) + \
   $(SOURCES) \
   $(L_SOURCES) \
   $(LPS_SOURCES) \
   $(LCS_SOURCES) \
   $(VLID_TEST_PATH)/V2p8p3_solar_lpcs_OMP_tester.f90
...
F90SOURCES_VSLEAVEPLUS := $(notdir $(filter %.f90, $(SOURCES_VSLEAVEPLUS)))
F90OBJECTS_VSLEAVEPLUS := $(patsubst %.f90, %.o, $(addprefix $(OBJ_PATH)/, $(F90SOURCES_VSLEAVEPLUS)))

F90SOURCES_Siewert2000 := $(notdir $(filter %.f90, $(SOURCES_Siewert2000)))
F90OBJECTS_Siewert2000 := $(patsubst %.f90, %.o, $(addprefix $(OBJ_PATH)/, $(F90SOURCES_Siewert2000)))
...
F90SOURCES_SOLAR_LPCS_OMP := $(notdir $(filter %.f90, $(SOURCES_SOLAR_LPCS_OMP)))
F90OBJECTS_SOLAR_LPCS_OMP := $(patsubst %.f90, %.o, $(addprefix $(OBJ_PATH)/, $(F90SOURCES_SOLAR_LPCS_OMP)))
...
#solar: v2p8p3_solar_tester.exe \
solar: pcrtm_omps_simulator.exe \
       v2p8p3_solar_lpcs_tester.exe

thermal: v2p8p3_thermal_tester.exe \
         v2p8p3_thermal_lpcs_tester.exe

brdf: v2p8p3_brdf_self_tester.exe \
      v2p8p3_brdfplus_tester.exe

obsgeo: v2p8p3_solar_lpcs_tester.exe \
        v2p8p3_brdfplus_tester.exe

doublet: obsgeo

Planetary: v2p8p3_Planetary_tester.exe

LWCoupling: v2p8p3_LWCoupling_tester.exe

openmp: v2p8p3_solar_OMP_tester.exe \
        v2p8p3_solar_lpcs_OMP_tester.exe

#@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@

pcrtm_omps_simulator.exe: $(F90OBJECTS_SOLAR)
	$(FC) $^ -o $@
v2p8p3_thermal_tester.exe: $(F90OBJECTS_THERMAL)
	$(FC) $^ -o $@
v2p8p3_brdf_self_tester.exe: $(F90OBJECTS_BRDF_SELF)
	$(FC) $^ -o $@
v2p8p3_solar_lpcs_tester.exe: $(F90OBJECTS_SOLAR_LPCS)
	$(FC) $^ -o $@
v2p8p3_thermal_lpcs_tester.exe: $(F90OBJECTS_THERMAL_LPCS)
	$(FC) $^ -o $@
v2p8p3_brdfplus_tester.exe: $(F90OBJECTS_BRDFPLUS)
	$(FC) $^ -o $@

# Note: These tests not included in "all" because they require different
#       "vlidort_pars.f90" settings

v2p8p3_vsleaveplus_tester.exe: $(F90OBJECTS_VSLEAVEPLUS)
	$(FC) $^ -o $@
v2p8p3_Siewert2000_tester.exe: $(F90OBJECTS_Siewert2000)
	$(FC) $^ -o $@
v2p8p3_Planetary_tester.exe: $(F90OBJECTS_FULL_PLANETARY)
	$(FC) $^ -o $@
v2p8p3_LWCoupling_tester.exe : $(F90OBJECTS_LWCoupling)
	$(FC) $^ -o $@

# Special OpenMP tests

v2p8p3_solar_OMP_tester.exe: $(F90OBJECTS_SOLAR_OMP)
	$(FC) $(FFLAGS_OPENMP) $^ -o $@
v2p8p3_solar_lpcs_OMP_tester.exe: $(F90OBJECTS_SOLAR_LPCS_OMP)
	$(FC) $(FFLAGS_OPENMP) $^ -o $@
